...
## 2026-08-17 세션 — 일요일 정기점검. 🔴🔴🔴 **노출붕괴 원인 특정: 유료 백링크 판매처를 향한 dofollow 배지(링크스팸)** + Bing 데이터 최초 확보로 "콘텐츠가 아니라 도메인 신뢰가 병목"임이 실증됨
...
### 🔴🔴🔴 최우선 — 노출붕괴 원인을 특정함. 08-06 사용자 커밋의 dofollow 배지가 유료 백링크 판매처를 가리키고 있었음

**08-11 세션까지 "원인 미상"이던 건이 이번에 거의 확정됐음.**
...
- **웹서치로 확인한 이 사이트의 정체**: 런치 디렉토리가 **아니라** "도메인 레이팅(Ahrefs DR)을 올려주는 유료 백링크 판매 서비스". 자사 소개문에 유료 백링크 제공을 명시하고 있고, vimeo/pinterest/disqus 등에 링크를 뿌리는 방식을 광고함. **즉 유료링크 판매처를 향한 dofollow 상호 배지링크 = 구글 링크스팸 정책의 '링크 스킴 참여' + '배지형 링크 교환' 패턴에 정면 해당.**
- **시점 상관이 매우 강함**:
...
**🔧 이번 세션 조치(커밋 `ba7a9c9`)**: 해당 배지에 `rel="nofollow sponsored noopener noreferrer"` 적용(다른 8개와 동일 값). href/img/style 미변경. 외부 배지 9개 전부 nofollow sponsored 확인.

**🔵 사용자가 판단·확인해야 할 것 (Claude가 코드로 못 고치는 영역)**:
1. **boostdomainrating.com 서비스를 실제로 구매했는지.** 구매했다면 그쪽이 **우리 사이트로 유료 PBN 백링크를 쏘고 있는 중**이고, 그게 진짜 원인일 가능성이 높음. 이건 HTML 수정으로 해결 안 됨 — **GSC → 링크 → 외부 링크**에서 최근 유입된 백링크 도메인을 확인하고, 스팸성이면 **구글 부인(disavow) 도구** 사용을 검토해야 함.
2. **GSC 수동 조치(Manual actions) 및 보안 문제 리포트 확인** — 링크스팸으로 수동 조치가 걸렸다면 화면에 뜸. 뜬다면 배지 제거 후 **재심사 요청**까지 해야 풀림. **캡처 1장이면 확인 가능하고, 이게 다음 세션 최우선.**
3. 배지 자체를 **아예 제거할지** 결정. nofollow로 정책 위반은 해소되지만, 링크 판매처와의 표시적 제휴 자체가 남는 게 신생 YMYL 사이트에 유리하진 않음. 삭제는 사용자 콘텐츠라 이번엔 손대지 않고 남겨둠.
4. **앞으로 홈에 배지·외부링크를 웹UI로 직접 추가할 때 반드시 `rel="nofollow sponsored"`를 붙일 것.** 이번 사고는 두 번 다 사용자 웹UI 커밋에서 시작됐음.
...
- 지난 세션 판단("제휴사 확보가 아니라 트래픽 병목 해소가 곧 수익화") 유지하되, **이번에 병목의 정체가 바뀜**: 막연한 "도메인 권위 부족"이 아니라 **구글 링크스팸 리스크라는 구체적·수정가능한 문제**로 좁혀짐. Bing에서 2~10위인 콘텐츠가 구글에서 71~100위라는 건, **구글 쪽 제약만 풀리면 되돌아올 여지가 있다는 뜻**이기도 함.
- 따라서 기대 임팩트 순위: ①**수동 조치 확인 + (있다면) 배지 제거·disavow·재심사** ②백링크 프로필 점검 ③기존 자산 밀도 보강(이번 calories 툴 같은 건) ④신규 발행.
- **iHerb 재신청·신규 제휴 신청은 이번에도 보류 권고.** 링크스팸 리스크가 미해소인 상태에서 제휴 심사를 넣으면 트래픽 부족에 더해 사이트 품질 이슈까지 겹쳐 반려 이력만 쌓임. **08-04부터 이월된 이 판단은 여전히 Opus 몫이며, 최소한 위 ①이 끝난 뒤로 미룰 것을 강하게 권고함.**

### 다음 세션 우선순위

1. **🔴 GSC 수동 조치(Manual actions) 화면 캡처 요청** — 링크스팸 수동 조치 여부 확인. 걸려 있으면 배지 제거 + 재심사 요청까지가 필수 경로. **1순위, 다른 것보다 먼저 물어볼 것.**
2. **🔴 GSC → 링크 → 외부 링크(상위 연결 사이트) 캡처 요청** — 07~08월에 유입된 스팸성 백링크가 있는지. 있으면 disavow 검토.
3. `ba7a9c9`(배지 nofollow) 이후 노출이 120회/일 수준으로 복귀하는지 추적. **2~3주 필요.** 복귀하면 원인 확정, 아니면 백링크 쪽이 진짜 원인.
...
- **⚠️ 2026-08-17 추가 — 외부 배지(우리가 남의 사이트로 나가는 링크)는 뱃지 정책이 아니라 링크스팸 문제임.** 홈에 외부 배지를 추가할 땐 **무조건 `rel="nofollow sponsored noopener noreferrer"`**. 이걸 빠뜨린 dofollow 배지 1개(boostdomainrating.com = 유료 백링크 판매처)가 노출 붕괴 원인으로 추정되는 사고가 있었음(상세는 2026-08-17 세션 기록). 사용자가 GitHub 웹UI로 직접 배지를 추가하는 경우가 많으므로 **매 세션 홈의 외부 링크 rel을 전수 점검할 것.**

...
- [ ] `waist-to-hip-ratio-calculator.html`, `vo2-max-calculator.html`, `active-recovery-calculator.html`, `calorie-deficit-calculator.html` 등 사이트맵엔 있지만 최근 3개월 서치콘솔 노출이 0인 페이지 다수 확인됨 — noindex/canonical 등 기술적 이슈는 없음을 확인함(정상, 페이지가 비교적 최근 것이거나 아직 크롤링/인덱싱 반영 전일 가능성 높음). Coverage 리포트의 "발견됨-미색인" 2건과 관련 있을 수 있으니 다음 체크 때 GSC에서 실제 URL 확인 권장(CSV 내보내기엔 URL 목록이 없어 이번 세션엔 특정 못함).
...
3. **Coverage**: 심각한 문제 "발견됨-미색인" 2건(유효성 검사 통과), 중요하지 않은 문제 0건 — 이전 세션과 동일하게 정상 범위로 판단. 색인 생성된 페이지 수는 86건으로 안정적.

...
- **🔵 세션 소스에서 중요한 사실**: bing/organic **10** + duckduckgo/organic **7**(DDG는 Bing 인덱스 기반) = **Bing 계열 17세션** vs **google/organic 4세션**. ahrefs.com/referral 10은 SEO 크롤러라 제외. **이 사이트에 실제로 사람을 보내는 건 구글이 아니라 Bing 계열이고 격차가 4배임.** 지금까지 모든 세션이 GSC(=구글)만 보고 판단해왔는데, 실유입 기준으론 Bing 최적화가 우선순위가 더 높을 수 있음. **다음 세션에 Bing Webmaster Tools 데이터를 받아볼 것을 강력 권장** — 지금 우리는 실트래픽의 80%를 아예 안 보고 있는 상태.
...
**2. IndexNow 키파일 배포 (같은 커밋)**
- 루트에 `4321c03c01df558ba66d3421da831297.txt`(개행 없는 raw 32자) 배포. **IndexNow 키: `4321c03c01df558ba66d3421da831297`** — 분실하면 재발급해야 하므로 이 문서에 보존.
- 대상 엔진: Bing / Yandex / Naver / Seznam / Yep. **구글은 IndexNow 미참여**(구글 쪽은 위 sitemap lastmod로 대응). 위 GA4 분석대로 실유입 주력이 Bing 계열이라 우선순위가 높다고 판단.
- **⚠️ 배포 확인 필요**: `https://mywellnesscalc.com/4321c03c01df558ba66d3421da831297.txt`가 200으로 열리는지 다음 세션에 확인할 것. Jekyll이 처리하는 구조라 루트 .txt는 통과할 것으로 보이나(llms.txt/robots.txt 전례) 실측은 안 해봄.

**3. 🔴 미완 — IndexNow 자동제출 워크플로가 PAT 스코프 문제로 푸시 거부됨**
- `.github/workflows/indexnow.yml`을 작성·검증(YAML 파싱 / run 블록 3개 `bash -n` / 임베드 파이썬 `py_compile` 전부 통과)했으나 푸시 시 거부:
  `refusing to allow a Personal Access Token to create or update workflow .github/workflows/indexnow.yml without workflow scope`
- 해당 커밋은 **롤백함**(로컬/원격 모두 `8c5cdea`로 동기, 미승인 잔여 변경 없음).
- **해결 방법 2가지 중 하나**: ①다음 토큰 발급 시 **`workflow` 스코프 포함**(fine-grained PAT면 Repository permissions → **Workflows: Read and write**) ②사용자가 GitHub 웹UI에서 직접 파일 추가(웹UI 커밋은 PAT 스코프 제약을 받지 않음). 워크플로 파일은 이번 세션에 사용자에게 별도 전달함.
- 워크플로가 없어도 키파일만 있으면 Bing Webmaster Tools에서 수동 제출은 가능함. 자동화가 안 될 뿐.
...
- **따라서 이번 세션의 수익화 우선순위 판단은 "제휴사를 더 뚫는다"가 아니라 "트래픽 병목을 푼다"임.** 그 관점에서 이번 세션 작업의 기대 임팩트 순위:
  ...
  2. **Bing 계열 정비**(IndexNow + Bing Webmaster Tools 확보) — 실유입 4배 채널을 지금까지 아예 안 보고 있었음.
  3. sitemap lastmod — 크롤 우선순위 신호 복구.
  4. 신규 콘텐츠 — **96%가 50위 밖인 상태에서 107번째 페이지의 한계수익은 0에 가까움.** 순위가 안 붙는 게 콘텐츠 부족 때문이 아니라 도메인 레벨 문제이기 때문.
...
2. **🔴 `workflow` 스코프 포함 PAT로 IndexNow 워크플로 배포 완료** (또는 사용자가 웹UI로 직접 추가). 아울러 키파일 URL 200 응답 확인.
3. **🔵 Bing Webmaster Tools 데이터 요청** — 실유입 4배 채널인데 지금까지 한 번도 안 봤음. 여기서 나오는 쿼리/순위는 GSC와 다를 수 있고, 그쪽이 오히려 공략 가능한 영역일 수 있음.
4. Coverage는 **드릴다운 말고 요약(색인/미색인 총계)**으로 받을 것 — 이번엔 총 색인 건수를 확인할 수 없었음.
5. canonical 통합(`weight-loss-plateau` 54→46.3) 계속 추적. **롤백하지 말 것.**
6. `how-much-fiber-per-day.html` 인바운드 보강(08-04) 효과 재확인 — 이번엔 7일차라 판단 유보.
7. GA4 봇 필터 설정(사용자 직접, 4세션 연속 미처리).
8. 신규 후보 2~3건 조사는 매 세션대로 계속. **단 퀴즈 양산 금지** — 위 "구조적 발견" 항목의 기대값 경고 참고.

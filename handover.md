...
- **교차 내부링크 반대 방향**: cookingcalcs-, myinsurancecalc- 프로젝트 채팅에서 진행 예정(프롬프트 전달함) — 완료 여부 다음 세션에 확인
- **뱃지 정리**: blog/index.html의 2026-06-28 날짜 뱃지 4개, 다음 체크 시점(14일 경과)에 제거 필요

## 다음에 할 만한 것 (우선순위 없이 나열, 데이터 보고 판단)

- [ ] 다음 서치콘솔 체크(1~2주 후 권장)에서 이번 세션 최적화/보강/버그수정 효과 확인
- [ ] 교차 내부링크 반대 방향 완료 여부 확인
- [ ] 뱃지 14일 규칙 재점검
- [ ] AdSense 재신청 여부는 트래픽 데이터 보고 판단 (아직 이르다고 판단됨)
- [ ] VO2max/단백질 타이밍 등 클러스터 완성용 신규 콘텐츠 추가 후보 있음(검색량 근거는 약함, 필요시 검토)
...
**작업 방식**: 사용자가 새 PAT 제공. 자료: GSC Performance + Coverage 2종(09-22, 차트 09-18까지), Bing Page/Keyword(09-22), GA4. 대시보드/시각화 금지. Bing은 판단 재료로만.

### 🔴 최우선 — 구글 "크롤링됨-미색인"에 핵심 페이지 2건이 들어감

| URL | 최종 크롤 | Bing 현황 |
|---|---|---|
| **`blog/healthy-body-fat-percentage.html`** | 2026-07-18 | **Bing 1위 페이지**(230노출/3클릭/7.2위) |
| `blog/vitamin-d-and-exercise.html` | 2026-07-09 | 20노출/0클릭/6.0위 |
| `favicon.ico` | 2026-07-09 | (무해) |

- 차트: 07-11부터 1건(favicon) → **09-05에 2건 → 09-15에 3건.** 즉 9월 들어 두 블로그 페이지가 차례로 이 상태로 분류됨.
- **기술 점검 결과 결함 없음**: 두 페이지 모두 canonical 자기참조 정상, `noindex` 없음, title/H1 정상, 분량 충분(3,898단어 / 1,972단어). 비타민D 클러스터 내 H2/H3 중복도 확인했으나 **겹치는 제목은 "Frequently asked questions" 보일러플레이트뿐**이라 자기잠식 근거 없음.
- **즉 기술적 차단이 아니라 구글의 품질/가치 판단임.** **원인은 미상 — 단정하지 말 것**(08-17 boostdomainrating 건에서 원인을 과잉 단정했다가 철회한 전례가 있음).
- **확실한 사실 하나**: `healthy-body-fat-percentage`의 판정은 **07-18 크롤본 기준**임. 이 페이지는 **09-08에 +875단어 보강 + 자기 표와 모순되던 수치 교정**을 했고 sitemap lastmod도 09-08로 갱신했지만, **구글은 그 이후 재크롤하지 않았음.** 즉 구글이 "색인할 가치 없다"고 본 건 **보강 이전 버전**임.
- **🔵 사용자 권장 조치(코드로는 불가)**: **GSC → URL 검사 → `healthy-body-fat-percentage.html` 입력 → "색인 생성 요청".** 판정 기준이 된 버전과 현재 버전이 크게 다르므로 재크롤을 유도할 근거가 명확함. **이건 구글 쪽 조치라 Bing 개입 금지 방침과 무관함.** `vitamin-d-and-exercise`는 07-09 이후 변경이 없어 재요청해도 결과가 같을 가능성이 높으니 우선순위 낮음.
- **⚠️ 노출 감소만으로 색인 탈락을 판단하지 말 것**: 이 페이지의 3개월 창 노출은 778 → 369(−53%)였지만, **색인 목록에 없는 `fat-burning-heart-rate-zone`도 951 → 435(−54%)로 같은 폭 감소**했음. 즉 노출 감소는 롤링 창 효과가 크고, 색인 상태 판단은 **Coverage 자료로만** 할 것(사용자 지시 "색인은 내가 준걸로만 판단"과도 일치).

### 🔴 비타민D 클러스터 연령별 상한(UL) 오류 교정 (커밋 `b02b6bf`)

**발견 경위**: Bing 무클릭 쿼리 `4000iu vitamin d daily requirement age range`(7노출/10위)가 연령 구간별 상한을 묻고 있어, 착지 후보인 `how-much-vitamin-d-do-you-need-daily`(Bing 141노출/1클릭, 클러스터 3위)를 점검함.

- 이 글의 표가 **"아동 1~13세 상한 2,500~3,000 IU"로 묶어 표기**하고 있었음. **NIH 공식 기준은 1~3세 2,500 / 4~8세 3,000 / 9~13세 4,000 IU** — 즉 **9~13세 상한을 과소 표기**. 영아도 0~12개월을 한 행으로 묶어 **7~12개월(1,500 IU)이 누락**돼 있었음.
- **클러스터 전체를 교차 대조하자 `tools/vitamin-d-calculator.html`(사이트 최대 클릭 페이지)의 기존 표에도 동일한 오류가 남아 있었음.** **09-01 세션에서 계산 로직과 신규 표는 NIH 기준으로 고쳤지만 같은 페이지의 원래 표는 손대지 않아서, 한 페이지 안에서 "9세 이상 4,000"과 "1~13세 2,500~3,000"이 3주간 서로 충돌하고 있었음.** 09-08에 등재한 "기존 수치도 그 페이지 표와 대조하라" 규칙이 생기기 전의 작업이라 놓친 것.
- **수정**:
  - `how-much-vitamin-d-do-you-need-daily`: 표 6행→9행(영아 0~6/7~12개월, 아동 1~3/4~8/9~13세 분리), **`Upper Limit (mcg)` 열 신설**(25/38/63/75/100), 표 아래에 **"4,000 IU 상한은 9세부터 적용된다"**는 안내 문단 추가(쿼리가 묻는 연령 구간을 정면으로 답함), FAQ 6→7개.
  - `vitamin-d-calculator`: 기존 표를 동일 기준으로 교정.
- **검증**: 계산기 페이지의 **표 2개 + 계산 로직(`age<=3→2500, <=8→3000`, 그 외 4000) 3자 일치** 확인. 사이트 전체에서 `1–13 years` / `2,500–3,000` / `Infants 0–12 months` **잔여 0건.**
- **⚠️ 교훈 추가(아래 품질기준에 등재)**: 수치를 교정할 때 **같은 페이지의 다른 표뿐 아니라 같은 클러스터의 형제 페이지까지 전수 grep**할 것. 09-01엔 새로 넣은 표만 맞추고 같은 페이지의 원래 표를 놓쳤고, 이번엔 그걸 형제 페이지 점검 중에 발견했음.

### 구글 Performance — 사실상 정체

- 3개월 누적: 클릭 **9**(직전 8), 노출 7,272(직전 8,848). 클릭 쿼리 표시 없음.
- **09-15 규칙 적용 — 개별 페이지 순위로 대조**: protein-calculator 77.2→77.1, zone-2-by-age 11.8→12.0, fat-burning 58.3→56.5, healthy-body-fat 83.6→82.9, **calories-burned-by-heart-rate 27.1→25.8**(유일하게 1위 이상 개선, 노출도 381→429 증가). **전반적으로 정체, 의미 있는 개선 없음.**
- 일별: **09-14~16에 노출 스파이크(143 / 170 / 150)** 후 다시 0~20. 08-22~24에 이은 두 번째 3일 스파이크. **원인 미상 방침 유지.**
- `detraining-calculator` 구글: 노출 3, 평균 순위 37.0(직전 2노출/5.5위). **노출 1건이 낮은 순위로 추가되며 평균이 끌려내려간 것**으로 표본이 너무 작아 추세로 읽을 수 없음.
- 신규 나머지 5건은 여전히 구글 미등장. Coverage "발견됨-미색인" 4건(sitting-offset, active-couch-potato, exercise-snack, which-exercise-snack) **09-18까지 그대로** — 08-29 등장 이후 약 3주째.
...
- 6주간(08-17 → 09-22) 노출 44 → 1,302(**29.6배**), 클릭 3 → 31.
- **09-15 보강 효과 가능성**: `best-time-to-exercise` 노출 35 → **86**, 클릭 4 → **5**(5.8위). 보강 1주 만에 노출 2.5배. (인과 단정은 금지, 다음 세션에 추이 재확인)
- **`detraining-calculator` Bing 성장 중**: 노출 5 → **21**, 클릭 1, **3.8위 유지.** 포맷 공백 전략의 유일한 실측 사례가 계속 커지고 있음.
- 신규 클릭 쿼리: `calorie deficit whilst drinking alcohol`(2위) / `if you are in a calorie deficit but some of those calories are alcohol...`(5위) → `alcohol-and-calorie-deficit` 작동 중. `why is it when i eat food high in protein my apetite is reduce`(6위) → `protein-satiety-appetite-control` 작동 중. 클릭원이 계속 분산되고 있음.
- **🔵 다음 보강 후보 — CTR 문제**: 무클릭 최다 쿼리 **`fat burn heart rate by age` 21노출 / 5.0위 / 클릭 0.** 착지는 `fat-burning-heart-rate-zone`(Bing 100노출/1클릭)으로 추정. 5위에서 21노출 0클릭은 **콘텐츠 결손이 아니라 타이틀/디스크립션 문제일 가능성**. 다음 세션에 한 주 더 보고 지속되면 메타 점검할 것.

### GA4 — 참여시간 반등(과대해석 금지)

- 4주: 활성 491명, 이벤트 1,673, **평균 참여시간 10.2초**(13.7 → 9.9 → 6.1 → 5.3 → **10.2**, 4주 연속 악화 후 반등).
- 싱가포르 348/491 = **70.9%**(직전 68.6% → 오히려 소폭 상승). **봇 필터 10세션 연속 미설정.** 봇 비중이 늘었는데 참여시간이 반등한 건 봇 트래픽 성격이 바뀌었을 가능성도 있어 **이 반등을 개선 신호로 읽지 말 것.**
- google/organic 0(4주 중 3주). Bing계열 약 38세션.
- 이탈률: `healthy-body-fat-percentage` **0.69**로 **4주 연속 콘텐츠 페이지 중 최저.** — 구글이 방금 색인에서 뺀 바로 그 페이지가 실사용자 참여는 사이트 최고라는 점이 아이러니하게 확인됨.

### 신규 콘텐츠 후보 — 1건 조사, 기각 (누적 58건 / 성공 2건)

| 후보 | 판정 |
|---|---|
| 식후 걷기(혈당) 계산기 | **기각 — 삼중.** ①아티클 포화(Wikipedia, Nature, NBC, Fox 외 다수) ②**당뇨·장수 도구 사이트들이 계산기 묶음으로 선점**(diabetesknow의 A1C 계산기, longevitymate의 longevity tools, timetowalk의 walking time calculator) ③**혈당 = YMYL**이고, 우리 `sitting-offset`(휴식 플랜)·`exercise-snack`과 **자기잠식 위험** |

**판단**: 이번 세션은 **정확성 교정(사이트 최대 클릭 클러스터의 자기모순 해소)**이 신규 발행보다 우선이라고 봤음. 전략 변경 아님.

### 💰 수익화 관점 우선순위

- 실유입은 **Bing계열 약 38세션**이 전부(구글 0). 그래서 Bing에서 작동 중인 비타민D 클러스터(클릭 최대)의 **정확성**을 최우선으로 처리함. **보충제 제휴를 재개할 경우 진입점이 이 클러스터인데, 아동 상한을 틀리게 안내하는 상태로 제휴 링크를 붙이면 신뢰·법적 리스크가 됨.** 이번 교정은 그 전제조건을 정리한 것.
- **제휴 보류 권고 유지**(사유: 트래픽 절대량).
- **`healthy-body-fat-percentage` 색인 탈락은 수익화 관점에서도 중요**: Bing 1위이자 GA4 참여 최고 페이지라, 구글 색인이 복구되면 구글 쪽 유입의 가장 유력한 후보가 됨. **사용자의 "색인 생성 요청" 1회가 이번 주 가장 비용 대비 효과가 큰 조치.**

### 다음 세션 우선순위

1. **`healthy-body-fat-percentage` 색인 복구 여부 확인** — 사용자가 색인 요청을 했다면 Coverage의 "크롤링됨-미색인" 목록에서 빠졌는지, 최종 크롤 날짜가 갱신됐는지 확인.
2. **"발견됨-미색인" 4건** — 약 3주째 대기. 다음 세션에도 그대로면 대기가 이 사이트 평균(2~3주)을 넘은 것이므로 따로 기록.
3. `fat burn heart rate by age`(21노출/5위/0클릭) 추이 확인 → 지속되면 `fat-burning-heart-rate-zone` 타이틀·디스크립션 점검.
4. `best-time-to-exercise`(09-15 보강) 노출 증가가 이어지는지 확인.
5. Bing·GSC Performance·Coverage 계속 요청. **포지션 분포 비율 단독 판단 금지 — 개별 페이지 순위 대조**(09-15 규칙).
6. GA4 봇 필터(사용자 직접, 10세션 연속 미처리).
7. **⛔ 하지 말 것**(누적): Bing 등록·IndexNow 등 제출 개입, 여성 연령별 체지방 신규 페이지(보강은 가능), 비타민D 식품 계산기·독성 단독 페이지, HRR 계산기, 러킹, 스웨트레이트, 디로드, 수면부채, 식품별 단백질 계산기, 지구력 탄수화물 계산기, 심박존별 칼로리 비교 도구, 사우나 계산기, **식후 걷기 계산기(이번 기각)**, 퀴즈 0노출 내부링크 재조사, H2 전수대조, DNS·Pages 빌드 가설, boostdomainrating 재론, puppeteer 실물 렌더 검증, **색인 탈락 원인 단정(이번 건 포함 — 원인 미상 유지)**.

---

## 2026-09-15 세션 — 주간 점검. 🟢 **`detraining-calculator`가 Bing에서 클릭까지 발생(3.8위) — 포맷 공백 전략 2차 검증** / 🔴 **지난주 "구글 순위 개선" 판단은 착시였음, 정정함** / Bing 5주 연속 성장

**작업 방식**: 사용자가 새 PAT 제공. 자료: GSC Performance + Coverage 2종(09-15), Bing Page/Keyword(09-15), GA4. 대시보드/시각화 금지. Bing은 확정 방침대로 판단 재료로만.

### 🔴 먼저 — 09-08 세션의 "구글 순위 분포 실질 개선" 판단을 정정함

09-08에 `51~100위 비중 95.5% → 90.7%`를 두고 **"4개월 만에 처음 실질 개선"**이라 적고, 수익화 판단(제휴 재신청 시점 재검토)의 근거로도 썼음. **이번 주 데이터로 보니 그 해석은 틀렸음.**

- 이번 주 51~100위 비중은 **90.6%** — 개선이 멈췄음(90.7 → 90.6).
- **개별 페이지 순위를 전수 대조하니 전부 제자리였음**(±0.6위 이내):

| 페이지 | 노출 변화 | 순위 변화 |
|---|---|---|
| protein-calculator | 2,267 → 1,931 (−336) | 77.4 → 77.2 |
| fat-burning-heart-rate-zone | 1,075 → 951 (−124) | 57.7 → 58.3 |
| healthy-body-fat-percentage | 888 → 778 (−110) | 83.8 → 83.6 |
| zone-2-heart-rate-by-age | 998 → 999 | 11.8 → 11.8 |
| one-rep-max | 310 → 272 | 83.6 → 83.6 |

- **원인**: 51~100위에 있는 **고노출 페이지들이 노출을 더 빠르게 잃으면서 분모가 줄어든 것**(쿼리 노출 총계 5,589 → 4,870). 순위는 아무것도 안 움직였는데 비율만 바뀐 **구성 변화(compositional shift)** 착시였음.
- **⚠️ 교훈으로 등재**: **포지션 분포 비율은 단독으로 읽지 말 것.** 3개월 롤링 창이라 붕괴일이 창에 들어오면서 노출이 계속 빠지는 국면에서는 비율이 저절로 움직임. **반드시 "개별 페이지 평균 순위"와 함께 대조해서, 순위가 실제로 내려갔는지 확인한 뒤 판단할 것.**
- **수익화 판단도 원복**: 09-08에 "개선 중이나 아직 이른 국면"이라 적었으나, 실제로는 **개선 근거가 없음.** 제휴 보류 사유는 이전과 동일하게 **트래픽 절대량**으로 되돌림.

### 🟢 포맷 공백 전략 2차 검증 — 이번엔 클릭까지 나옴

- **`tools/detraining-calculator.html`이 Bing에 등장: 노출 5, 클릭 1, 순위 3.8위.**
- **클릭 쿼리가 정확히 그 주제**: `detraining strength loss after 2 weeks break returning to gym how much strength muscle l...`(4위). 08-24(1차)에 "아티클은 15개+ 포화지만 계산기가 영어권에 0건"이라 판단해 만든 페이지가, **바로 그 롱테일 자연어 쿼리로 클릭을 받은 것.**
- 구글 쪽은 노출 2 / 순위 5.5위로 지난주와 동일(변화 없음).
- **나머지 5건은 여전히 미등장**: `return-to-training-after-break`, `sitting-offset-calculator`, `active-couch-potato`, `exercise-snack-calculator`, `which-exercise-snack` — 구글·Bing 양쪽 모두.
- **Coverage는 판단 불가**: 이번에 받은 2종 모두 차트가 **09-04까지로 지난주 자료와 동일**함. "발견됨-미색인" 4건(sitting-offset, active-couch-potato, exercise-snack, which-exercise-snack) 그대로이고, 추가로 받은 "크롤링됨-미색인"은 `favicon.ico` 1건뿐(무해). **다음 세션엔 09-05 이후가 포함된 Coverage를 받아야 신규 4건 판정이 가능함.**
- **현 시점 정리**: 신규 6건 중 **1건이 양쪽 엔진에서 색인·순위 확보, 그중 Bing에서 클릭 1건.** 표본은 작지만 **가설의 방향은 맞았음.** 전략 유지.
...
- 5주간 노출 21.7배, 클릭 8.7배. 1~3위 89노출 / 4~10위 535노출 / **11위 밖 단 1노출**.
- 상위: `healthy-body-fat-percentage`(174/3클릭/7.2위 — 5주간 55→87→138→174 연속 성장), `vitamin-d-calculator`(158/**7클릭**/6.6위), `how-much-vitamin-d-do-you-need-daily`(110/1클릭), `protein-calculator`(101/2클릭), **`best-time-to-exercise`(35/4클릭/6.4위 — 지난주 25노출 0클릭에서 급등)**.
- **비타민D 클러스터가 26클릭 중 8건(31%)** — 비중은 내려갔지만 여전히 최대 단일 클러스터. 클릭원이 체지방·단백질·운동시점으로 분산되는 중.
- **🟢 과거 보강 3건이 전부 클릭 유지**: `calculator to avoid vitamin d toxicity`(1위, 09-01 보강), `what calories does zone 1 burn per day`(3위, 08-24 2차 보강), `what is a healthy body fat percentage, body muscle percentage, hydration percentage`(1위, 09-08 보강). **"Bing 클릭 쿼리의 결손을 메운다" 기준이 3회 연속 결과를 유지 — 이 기준은 계속 쓸 것.**

### GA4 — 참여시간 4주 연속 악화, 봇 문제 심화

- 4주: 활성 503명(직전 495), 이벤트 1,658, **평균 참여시간 5.3초**(13.7 → 9.9 → 6.1 → **5.3**, **4주 연속 악화**).
- **싱가포르 345/503 = 68.6%.** Shenzhen 17, San Jose 15, Warsaw 13, Paris 10. **봇 필터 9세션 연속 미설정.**
- **google/organic 0**(3주 중 2주가 0). Bing계열 42세션(bing 11 + ddg 11 + yahoo 10 + ecosia 4 + cn.bing 3 + uk.yahoo 3)으로 계속 증가. AI 2(perplexity).
- 이탈률: `healthy-body-fat-percentage` **0.73**으로 3주 연속 콘텐츠 페이지 중 최저(나머지 0.85~1.00).
- **⚠️ 참여시간이 13.7 → 5.3초로 넉 달 만에 3분의 1이 됐음.** 실사용자가 늘고 있는데(Bing 클릭 26) 평균이 내려가는 건 **봇 유입이 그보다 빠르게 늘고 있다는 뜻**이고, 이제 GA4 수치 자체를 판단에 쓰기 어려운 수준임. **봇 필터 설정이 "하면 좋은 일"에서 "데이터 신뢰도 문제"로 넘어갔음.**

### 이번 세션에 한 작업 — 보강 1건 (커밋 `6ce2a90`)

**`blog/best-time-to-exercise.html` 1,408 → 2,351단어**

- **선정 근거**: Bing 노출 25→35, **클릭 0→4**로 이번 주 2번째 최대 클릭원이 됨(6.4위). 동시에 1,408단어로 블로그 품질기준(1,500) 미달.
- **🔴 결손 진단이 이번 건의 핵심**: 클릭 쿼리 3건이 **전부 `research`/`science`를 명시**함(`best time of day to exercise research morning vs evening workout` 4위 등). 그런데 본문은 `research` 11회·`studies` 3회를 언급하면서 **정량 수치가 `%` 0건, `participants` 0건, `trial` 0건** — "연구에 따르면"만 반복하고 숫자를 하나도 제시하지 않는 상태였음. **쿼리가 요구하는 것과 정반대.**
- **추가 내용(전부 웹서치로 출처 확인한 수치만 사용, 창작 없음)**:
  - 성능 차이의 크기: Wingate 반복측정 일중 진폭 **peak power 7.6% / mean power 11.3%**(아침 최저, 늦은 오후~초저녁 최고). 활동적 여성 07:00 vs 16:00 비교에서 peak/mean power·CMJ·SJ 점프높이·균형·주의력 모두 오후 우위.
  - 수개월 결과는 차이 없음: 42명 24주 시험에서 아침/저녁군 모두 레그프레스 1RM 유사 증가(**아침측정 14~19%, 저녁측정 18~24%**), 근단면적 12~20%. 체계적 문헌고찰도 동일 결론. **단 아침 훈련이 일중 곡선을 평탄화시켜 고정 시간대 테스트/경기에는 그 시간대 훈련이 실이익**이라는 점 명시(실용적으로 가장 쓸모 있는 부분).
  - 타이밍이 실제로 갈리는 지점: 과체중 남성 무작위시험에서 **저녁군만** 공복혈당·인슐린·콜레스테롤·TG·LDL 개선(VO2peak는 양군 동일), 활동적 성인의 **성별 분화**(여성은 아침이 복부지방·혈압 / 저녁이 근기능, 남성은 저녁이 혈압), 운동후 저혈압은 저녁이 큼. **단일 시험임을 명시하고 처방이 아니라 방향으로 읽으라는 단서를 붙임.**
  - **아침/저녁 결과별 비교표 7행 신규.** "차이 크기" 열을 넣어, **신뢰할 만큼 큰 차이가 나는 행은 '지속성' 하나뿐**이라는 결론이 표에서 바로 읽히도록 구성.
  - FAQ 6→9개.
- **⚠️ 반응형 — 08-24 사고 조건과 동일해서 세트로 먼저 처리함**: 이 페이지엔 **table 관련 CSS도 `.article-main{min-width:0}`도 둘 다 없었음.** 표를 그냥 넣으면 그리드 아이템이 축소되지 않아 페이지 전체가 가로로 터지는 조건이었으므로, `table-wrapper` / `data-table` / `article-main{min-width:0}`을 **한 세트로 먼저 추가한 뒤** 표를 삽입.
- **수치 정합성**: 본문·FAQ·JSON-LD 3곳에서 7.6/11.3, 14–19/18–24, 12–20% 각 3회 일치 확인. 표의 "8–11%"가 본문 7.6/11.3과 정합함도 확인.

### 신규 콘텐츠 후보 — 2건 조사, 전부 기각 (누적 57건 / 성공 2건)

| 후보 | 근거 | 판정 |
|---|---|---|
| 심박존별 칼로리 소모 비교 도구 | Bing 무클릭 4노출 `which heart rate zone burns the most calories`(6위) | **이중 기각.** ①외부 포화 — Omnicalculator, WebMD, WHOOP, GymCreek, runbundle, a2zcalculators, onlinefreecalculators 등 ②**자기잠식** — 우리 `fat-burning-heart-rate-zone`·`calories-burned-by-heart-rate-zone`이 이미 이 질문을 다룸 |
| 사우나 세션 시간/빈도 계산기 | 사이트에 사우나·열노출 콘텐츠 전무, 미개척 | **기각 — 양 포맷 포화.** `peakprimalwellness`에 **"Sauna Session Calculator 2026" 전용 계산기**, mammothmug에 사우나 수분 계산기. 아티클은 Peak Saunas·UseSauna·sauna.guide·My Fitness Outlet·Mammoth Mug 등 8개+이고 **대부분 사우나 판매업체 소유**라 갱신 동기·자본이 우리보다 큼(지구력 탄수화물 계산기와 동일한 구조) |

**판단**: 신규 발행 0건. **전략 변경 아님** — 포맷별 공급 분리 집계 기준을 정상 적용했고 계산기 포맷까지 채워져 있었음. 사용자 기준("진짜 할 게 없으면 안 하는 게 맞다") 적용.

### 💰 수익화 관점 우선순위

- 실유입은 **Bing계열 42세션 + AI 2**가 전부(구글 오가닉 0). 그래서 이번에도 **"Bing에서 클릭 나는 페이지의 결손 보강"**이 1순위였고, 이번 주 클릭이 0→4로 급등한 페이지를 골랐음. **이 선정 기준은 3주 연속 결과를 유지하고 있어 계속 쓸 것.**
- **제휴 보류 권고 유지.** 사유는 위 정정대로 **트래픽 절대량**으로 되돌림(09-08의 "개선 중" 판단은 근거 없었음).
- **다만 방향 자체는 나쁘지 않음**: Bing 클릭이 3→26으로 5주 연속 늘고, 클릭원이 비타민D 단일 의존(55%)에서 31%로 분산되는 중임. **클러스터 3~4개가 각각 클릭을 내는 구조로 가고 있어, 이 추세가 이어지면 제휴 심사에 쓸 수 있는 "안정적 오가닉 유입" 서사가 만들어짐.** 다음 2~3세션 Bing 추이가 판단 시점.

### 다음 세션 우선순위

1. **Coverage를 09-05 이후 데이터가 포함된 것으로 요청** — 이번엔 두 파일 모두 09-04까지라 신규 4건 색인 판정이 불가했음. (※ **구글 서치콘솔** 자료)
2. **신규 5건 등장 여부 추적**(구글·Bing 양쪽). `detraining-calculator` 외 5건이 3~4주째 미등장이므로, 다음 세션에도 안 나오면 **"발행 순서대로 색인되는지(큐)" 아니면 "개별 페이지 문제인지"를 구분해볼 것.** `detraining`은 08-24 발행분 중 하나였고 같은 날 발행된 `return-to-training`은 아직 미등장이라, 페이지 단위 차이일 가능성이 있음.
3. **Bing 리포트 계속 요청**(5주 연속 성장, 실제 클릭 위치를 알려주는 유일한 데이터).
4. GSC Performance 계속 요청. **단 포지션 분포 비율만 보고 개선/악화를 판단하지 말 것** — 반드시 개별 페이지 순위와 대조(위 정정 참고).
5. **GA4 봇 필터** — 참여시간 4주 연속 악화(13.7→5.3초)로 데이터 신뢰도 문제가 됐음. 9세션 연속 미처리.
6. **⛔ 하지 말 것**(누적): Bing 등록·IndexNow 등 제출 개입, 여성 연령별 체지방 신규 페이지(3회 기각, 보강은 가능), 비타민D 식품 계산기·독성 단독 페이지, HRR 계산기, 러킹, 스웨트레이트, 디로드, 수면부채, 식품별 단백질 계산기, 지구력 탄수화물 계산기, **심박존별 칼로리 비교 도구(이번 기각, 자기잠식)**, **사우나 계산기(이번 기각)**, 퀴즈 0노출 내부링크 재조사, H2 전수대조, DNS·Pages 빌드 가설, boostdomainrating 재론, puppeteer 실물 렌더 검증(컨테이너 불가).

---

## 2026-09-08 세션 — 주간 점검. 🟢🟢 **포맷 공백 전략 첫 검증 성공: `detraining-calculator`가 구글 색인되어 5.5위** + ~~구글 순위 분포 4개월 만에 실질 개선~~ + Bing 4주 연속 성장

> **⚠️ 2026-09-15 정정**: 이 세션의 "구글 순위 분포 실질 개선" 판단은 **틀렸음.** 51~100위 비중 하락(95.5→90.7%)은 순위 개선이 아니라, 51~100위의 고노출 페이지들이 노출을 더 빠르게 잃어 분모가 줄어든 **구성 변화 착시**였음(개별 페이지 순위는 전부 ±0.6위 이내로 제자리). 아래 본문에서 이 항목과, 그것을 근거로 한 수익화 문단("개선 중이나 아직 이른 국면")은 **무효로 읽을 것.** 상세는 2026-09-15 세션 기록 참조.

**작업 방식**: 사용자가 새 PAT 제공. 자료: GSC Performance + Coverage(09-08), Bing 웹마스터 Page/Keyword(09-08), GA4. 대시보드/시각화 금지. Bing은 확정 방침대로 판단 재료로만 사용.

### 🟢🟢 최우선 보고 — 신규 6건 중 첫 색인 확인, 그리고 순위가 5.5위

09-01(2차)에 "신규 6건이 미색인 대기 중이고 포맷 공백 전략이 아직 한 건도 검증되지 않았다"고 남겨둔 건에 대한 답이 나왔음.

- **`tools/detraining-calculator.html`이 GSC Performance에 처음 등장 — 노출 2, 평균 순위 5.5위.** 08-24(1차) 발행분으로, 발행 약 3주 만에 색인·노출됨.
- **Coverage "발견됨-미색인" 목록에서 3건이 빠졌음**: `quiz/cardio-type.html`(07-25부터 5주 이상 묶여 있던 건), `tools/detraining-calculator.html`, `quiz/return-to-training-after-break.html`. 대신 그 이후 발행분 4건이 새로 들어옴(`sitting-offset-calculator`, `active-couch-potato`, `exercise-snack-calculator`, `which-exercise-snack`, 전부 최종크롤 `1970-01-01`).
- **해석**: 이 사이트는 신규 페이지가 **발견 → 대기 → 색인** 순으로 진행되며 **대기 구간이 대략 2~3주**인 것으로 보임. 09-01(2차)에 "6건이 계속 무색인이면 발굴보다 색인 문제로 초점을 옮길지 판단하라"고 남겼는데, **그럴 필요 없음 — 파이프라인은 작동하고 있고 단지 느릴 뿐임.** 발굴 전략 유지가 맞음.
- **다만 검증된 건 아직 1건뿐이고 노출도 2회**임. `return-to-training-after-break`는 미색인 목록에서 빠졌으나 Performance에는 미등장(색인됐지만 노출 0이거나 다른 상태). 나머지 4건은 여전히 대기. **다음 세션에 이 5건의 후속을 계속 추적할 것.**
- **순위 5.5위는 주목할 값**: 사이트 전체 평균이 60~70위대인데 신규 툴이 첫 노출부터 5.5위로 잡혔음. "경쟁사가 안 만드는 포맷으로 들어가면 순위가 쉽게 붙는다"는 가설과 정합적. **다만 노출 2회짜리 표본이므로 과대해석 금지.**

### 🟢 구글 — 순위 분포가 4개월 만에 처음 실질 개선

| 지표 | 08-24 | 09-08 |
|---|---|---|
| 클릭 / 노출 | 6 / 10,786 | **8 / 9,825** |
| 1~10위 | 0.2% | 0.3% |
| 31~50위 | 3.6% | **8.1%** |
| 51~100위 | **95.5%** | **90.7%** |

- **51~100위 비중이 95.5% → 90.7%로 처음 의미 있게 내려왔고, 31~50위가 3.6% → 8.1%로 배 이상 늘었음.** 4개월간 96% 근처에 고정돼 있던 지표라 노이즈로 보기 어려움.
- 클릭도 6 → 8로 증가. 상위 페이지: `protein-calculator`(2,267노출/77.4위), `fat-burning-heart-rate-zone`(1,075/57.7), **`zone-2-heart-rate-by-age`(998노출/11.8위/2클릭 — 사이트 최고 순위 페이지 유지)**, `healthy-body-fat-percentage`(888/83.8), `calories-burned-by-heart-rate`(384/**27.0위**).
- **08-22~24에 노출 스파이크**: 일별 139 / **388(1클릭)** / 80. 그 전후는 한 자릿수. 08-07 이후 이어지던 저조 구간에서 3일간 튀었다가 다시 내려갔음. **원인 미상 방침 유지 — 새 가설 세우지 말 것.** 다만 "완전히 죽은 게 아니라 간헐적으로 살아난다"는 점은 기록해둠.
- 09-01·09-04·09-05는 노출 1~4에 순위 1~5위로 찍힘(표본이 작아 순위가 튀는 구간).

### 🟢 Bing — 4주 연속 성장, 비타민D가 클릭의 42%
...
- 4주간 노출 15.8배, 클릭 6.3배. 포지션 1~3위 71노출 / 4~10위 396노출 / **11위 밖 단 1노출**.
- 상위: `healthy-body-fat-percentage`(138노출/**3클릭**/7.2위 — 4주간 55→87→138로 계속 성장), `vitamin-d-calculator`(106/**7클릭**/6.3위), `how-much-vitamin-d-do-you-need-daily`(87/1클릭/6.8위), `protein-calculator`(53/2클릭/7.6위), `active-recovery-calculator`(21/**3.3위**).
- **비타민D 클러스터가 19클릭 중 8건(42%)** — 3주 연속 최대 클릭원. 09-01(1차)에 "제휴 재개 시 1순위 진입점"으로 적어둔 판단이 유지됨.
- **🟢 지난 보강 2건이 클릭을 유지하고 있음**: `calculator to avoid vitamin d toxicity`(**1위, 클릭** — 09-01 보강 대상), `what calories does zone 1 burn per day`(**3위, 클릭** — 08-24 2차 보강 대상). **"Bing 클릭 쿼리의 결손을 메운다" 방식이 2회 연속 결과를 유지한 것으로, 이 선정 기준은 계속 쓸 것.**

### GA4 — 봇 비중·참여시간 계속 악화

- 4주: 활성 495명(직전 471), 이벤트 1,648, **평균 참여시간 6.1초**(직전 9.9초 → **계속 악화**).
- **싱가포르 333/495 = 67.3%.** Shenzhen 25, San Jose 16, Warsaw 13, Paris 10. **봇 필터 8세션 연속 미설정**(사용자 직접 작업).
- 소스: direct 458 / **Bing계열 34**(bing 14 + yahoo 7 + ddg 6 + ecosia 5 + cn.bing 2) / AI 3(perplexity 2 + chatgpt 1) / **google organic 1**(2주간 0이었다가 복귀) / ui.awin 4 / boostdomainrating 2.
- **이탈률**: 홈 0.92, **`healthy-body-fat-percentage` 0.72로 콘텐츠 페이지 중 최저**(나머지 0.86~1.00). 09-01(1차)에 보조 지표로 쓰라고 등재한 이탈률이 이번엔 Bing 1위 노출 페이지와 동일한 페이지를 지목함 — **두 지표가 2주 연속 같은 페이지를 가리키고 있어 선정 근거로 신뢰할 만함.**

### 이번 세션에 한 작업 — 보강 1건 (커밋 `56fecd8`)

**`blog/healthy-body-fat-percentage.html` 3,023 → 3,893단어**

- **선정 근거**: Bing 노출 1위(138/3클릭) + GA4 이탈률 최저(0.72) + 구글 888노출(83.8위로 개선 여지 큼). **여성 연령별 체지방 "신규 페이지"는 2회 기각된 항목이므로, 확립된 원칙대로 기존 페이지 보강으로 처리함.**
- **Bing 쿼리가 지목한 결손 2가지**:
  1. `what is a healthy body fat percentage, body muscle percentage, hydration percentage, for a...`(**1위, 클릭**) — 스마트체중계의 나머지 수치를 묻는 질문인데 본문에 `muscle percentage` 0회, `body water` 0회, `bone mass` 0회, `bioimpedance` 0회로 **전혀 다루지 않고 있었음.**
  2. `what is a good total body fat % of a 54 year old post menopausal woman`(5위, 클릭) — `postmenopausal` 0회.
- **추가**: H2 "스마트체중계의 나머지 숫자" 신설(TOC 반영) — 모든 수치가 임피던스 1회 측정에서 파생된다는 원리, **근육률이 체지방률의 거울상으로 움직이는 이유**(그래서 절대값은 의미 없음), 수분율이 다른 모든 수치의 최대 오차원(3~5%)이라는 점, 골량은 골밀도가 아니고 "대사나이"는 임상 정의가 없다는 점, 재현 가능한 측정 조건. FAQ 11→14개.
- **🔴 정확성 교정 — 페이지가 자기 표와 모순되고 있었음**: 기존 FAQ가 여성 기준을 `40–59세 23–33%, 60+ 24–35%`로 안내하는데, **같은 페이지 본문 표는 40–59세 22–32%(fitness 22–25 + acceptable 26–32), 60+ 23–33%**였음. 즉 **"55세 여성 체지방률" 계열 쿼리가 이 페이지 클릭의 주력인데 정작 우리 차트와 다른 숫자를 답하고 있었음.** 표 기준으로 통일했고, 이번에 추가한 폐경 FAQ도 초안에 기존 오류값을 그대로 옮겨 썼던 것을 함께 교정. 본문·FAQ·JSON-LD 3곳 반영 확인, `24–35` 잔여 0건.
  - **⚠️ 교훈**: 지금까지 표-로직 검증은 "이번 세션에 내가 새로 넣은 수치"만 대조해왔음. **이번 건은 기존 페이지가 원래부터 자기 표와 어긋나 있던 케이스**였고, 내가 그 오류값을 그대로 인용하면서 확대될 뻔했음. **기존 페이지를 보강할 때는 내가 추가한 수치뿐 아니라 인용하려는 기존 수치도 그 페이지 표와 대조할 것.**

### 신규 콘텐츠 후보 — 2건 조사, 전부 기각 (누적 55건 / 성공 2건)

Bing 클릭 쿼리에서 수요가 확인된 축을 우선 조사:

| 후보 | 근거 | 판정 |
|---|---|---|
| 식품별 단백질 함량 계산기 | Bing 클릭 쿼리 `how many protien grams per serving list salmon, egg, greek yogurt, and legumes`(2위, 클릭), `is 48g of protein enough for a 60kg person`(4위, 클릭) | **기각 — 전용 도구 포화.** **myproteincalc은 사이트 전체가 이 주제**(식품 차트 + 계산기 + 카테고리별 하위페이지), MissVickie 단백질 식품 계산기, proteinrate, proteinjug, CSPI, 병원 PDF 다수 |
| 지구력 운동 중 탄수화물 섭취(g/시간) 계산기 | 사이트에 지구력 영양 콘텐츠 전무, 미개척 영역 | **기각 — 전용 계산기 8개+.** nduranz, 42Cal, yearroundrunning, TriWorldHub, truefuels, trainingzones.io, endu1, Precision Hydration. 게다가 **대부분이 자사 젤·보충제를 파는 브랜드 소유**라 콘텐츠 갱신 동기와 자본이 우리보다 큼 |

**판단**: 신규 발행 0건. **전략 변경 아님** — 두 건 모두 08-24(3차)에 표준화한 "포맷별 공급 분리 집계"를 정상 적용했고, 계산기 포맷까지 이미 채워져 있어 진입로가 없었음. 사용자 명시 기준("진짜 할 게 없으면 안 하는 게 맞다") 적용.

### 💰 수익화 관점 우선순위

- 실유입은 여전히 **Bing계열 34세션 + AI 3세션**이 전부(구글 오가닉 1). 그래서 이번에도 **"Bing에서 이미 클릭 나는 페이지의 결손 보강"**을 1순위로 잡았고, Bing 노출 1위 + GA4 이탈률 최저가 동시에 지목한 페이지를 골랐음.
- **비타민D 클러스터가 3주 연속 Bing 클릭의 40%대**를 만들고 있음. 제휴(보충제 계열) 재개 시 진입점은 변함없이 여기.
- **제휴 신청은 이번에도 보류 권고.** 다만 근거가 한 가지 바뀜: 지금까지는 "트래픽 절대량"만 사유였는데, **이번에 구글 순위 분포가 처음 개선됐고 신규 페이지 색인 파이프라인도 작동이 확인됨.** 즉 "개선 중인데 아직 이른" 국면으로 볼 여지가 생겼음. **다음 2~3세션 동안 51~100위 비중이 계속 내려가고 신규 색인이 이어지면, 그때 제휴 재신청 시점을 재검토할 것.** (이 판단은 여전히 Opus 몫)

### 다음 세션 우선순위

1. **신규 5건 색인 후속 추적** — `return-to-training-after-break`(미색인 목록에서 빠졌으나 Performance 미등장)와 09-01(2차)·08-24(3차) 발행 4건. **발견→색인 대기가 2~3주로 확인됐으므로 다음 세션이 판정 시점.**
2. `detraining-calculator` 노출이 2회에서 늘어나는지 확인. **포맷 공백 전략의 유일한 실측 증거이므로 이 페이지 추이가 향후 발굴 규모를 좌우함.**
3. 구글 51~100위 비중이 90.7%에서 더 내려가는지 확인. **개선이 이어지면 수익화 판단(제휴 재개 시점)에 직결됨.**
4. Bing 리포트 계속 요청(4주 연속 성장). GSC Performance도 계속 요청.
5. Coverage는 가능하면 **요약(색인/미색인 총계)** — 6세션 연속 드릴다운. (※ **구글 서치콘솔** 자료임)
6. GA4 봇 필터(사용자 직접, 8세션 연속 미처리). 참여시간이 13.7→9.9→6.1초로 3주 연속 악화 중이라 봇 유입이 늘고 있을 가능성.
7. **⛔ 하지 말 것**(누적): Bing 등록·IndexNow 등 제출 개입, 여성 연령별 체지방 **신규 페이지**(3회 기각 — 보강은 계속 가능), 비타민D 식품 계산기·독성 단독 페이지, HRR 계산기, 러킹, 스웨트레이트, 디로드 계산기, 수면부채 계산기, **식품별 단백질 계산기(이번 기각)**, **지구력 탄수화물 계산기(이번 기각)**, 퀴즈 0노출 내부링크 재조사, H2 전수대조, DNS·Pages 빌드 가설, boostdomainrating 재론, puppeteer 실물 렌더 검증(컨테이너 불가).

---

## 2026-09-01 세션 (2차, 신규 발굴) — ⭐ **운동 스낵/VILPA 클러스터 신설.** "아티클 포화 / 툴·퀴즈 공백" 패턴 **3번째** 적용

**작업 방식**: 사용자가 새 PAT 제공. 신규 발굴 전용 세션. 지시는 08-24(1·3차)와 동일(폭넓은 키워드 → 리스트 → 경쟁강도 → 강하면 롱테일 / 가능하면 클러스터 / 문서수 적고 관심 많은 것 / 대시보드·시각화 금지).

### ⭐ 뚫은 자리 — "하루 몇 분의 짧은 고강도 버스트로 충분한가"

| 포맷 | 공급 | 판정 |
|---|---|---|
| 아티클 | The Conversation, Cleveland Clinic, UPMC, BMJ Group, Psychology Today, BBC Science Focus, CBC, NBC News, Yahoo Health, WCRF, FoundMyFitness, Colorado State KRNC 등 **12개+** | 포화 |
| **계산기** | **0건.** 검색어에 `calculator quiz tool`을 명시해 재검색해도 결과가 **전부 아티클**이었음 | **공백 → 진입** |
| **퀴즈** | **0건** | **공백 → 진입** |

- **08-24(1차) 디트레이닝, 08-24(3차) 좌식시간에 이어 같은 패턴의 3번째 적용.** 3회 연속 재현됐으므로 이 절차는 이제 표준으로 봐도 됨.
- **자기잠식 없음(맥락까지 확인)**: `exercise snack|VILPA|short burst|micro workout` grep 히트 4건은 전부 부수 언급이었음 — `zone-2-vs-hiit`의 HIIT 인터벌 묘사, `heart-rate-zone`의 Zone 5 설명, `zone-2-fitness-level` 퀴즈의 **선택지 문구**("Only in short bursts"). 08-24(3차)에 등재한 "히트가 나와도 주제인지 라벨인지 확인" 규칙이 이번에도 유효했음.
- **인접 클러스터와의 축 구분**: 6일 전 만든 `sitting-offset-calculator`는 *좌식시간 대비 중강도 목표*, 이번 것은 *고강도 버스트의 독자적 용량-반응*. 서로 상호링크하면서 본문에서 명시적으로 구분해둠.
- **기각 후보 2건**: **수면부채 계산기** — Pearson, calculatemysleep, dietplanner, smartsleepcalc, freefitnesscalculators, sleepcalcs, ultimatefinancecalculator 등 **전용 계산기 7개+** 포화(참고로 sleepcalcs는 jet lag·카페인 컷오프 계산기까지 보유 — 해당 인접 주제도 사실상 막혀 있음). **스웨트레이트**는 이미 09-01(1차)에 기각됨.

### 신규 페이지 2건

**1. `tools/exercise-snack-calculator.html` (1,548단어, FAQ 7) — 클러스터 앵커**
- **핵심 차별화**: 다른 활동 도구는 전부 주간 가이드라인(150분 중강도/75분 고강도)에 대비시킴. 그 프레임에서는 3×1분 = 주 21분이라 **버스트가 무의미해 보임.** 그런데 실제 버스트 연구는 가이드라인이 아니라 **"아무것도 안 하는 것" 대비로** 측정했고, 훨씬 낮은 용량에서 연관을 보고했음. 그래서 이 도구는 **연구가 실제로 쓴 단위인 "하루 누적 고강도 분"으로 보고**함. 이게 경쟁 문서와 갈리는 지점.
- 근거 수치: 2022년 약 25,000명 웨어러블 코호트(하루 1~2분 ×3회 → 전체사망·암사망 38~40%↓, 심혈관사망 48~49%↓), 4.4분/일 구간(26~34%↓), 2023년 암 분석(3.4~3.6분/일부터), BJSM 종설의 운동스낵 정의(5분 이하, 1일 2회+, 주 3~7일, 4~12주 → 심폐체력 개선), 계단 5회/일 8주 17% 개선.
- 출력: 5개 밴드 분류 + 기회유형별(계단/야외/실내/관절제한) 실행 플랜.
- **🟢 표-로직 불일치 사전 검출·수정**: 초안 표의 **밴드 라벨 2건**이 실제 분기와 어긋났음(3×1분=3.0분은 로직상 `Core studied band`인데 표는 `Entry band`, 5×3분=15분은 로직상 `This is training, not snacking`인데 표는 `Above studied range`). 5행 전부 계산 출력값으로 동기화. **수치(일일/주간)는 초안부터 일치했고 라벨만 틀렸던 케이스** — 앞으로 표 검증 시 숫자뿐 아니라 **분류 라벨도 함께 대조할 것.**
- 30개 조합 시뮬레이션으로 5개 밴드 전부 도달 및 경계값(1.5/3/5/10분) 확인.

**2. `quiz/which-exercise-snack.html` (1,081단어, FAQ 5)**
- **자기잠식 회피 설계가 이 페이지의 핵심**: 6일 전 만든 `active-couch-potato`는 *좌식시간+운동량*으로 **패턴을 분류**하는 퀴즈. 이번 것은 *공간/제약/장비/트리거*로 **실행 포맷을 추천**하는 퀴즈. **질문 5개가 전부 다른 축이라 사용자가 두 퀴즈를 다 봐도 겹치는 느낌이 없음.** (FFMI 3중복·plateau 2중복 사고를 반복하지 않기 위한 의도적 설계)
- 5개 결과(계단 / 앉았다일어서기 / 자전거 스프린트 / 빠른걷기 / 맨몸). 연구에서 **65세 미만은 계단, 고령층은 하지 근력·태극권이 우세**했던 실제 분포를 결과 배분에 반영.
- **1,024개 조합 전수 시뮬레이션: 5개 결과 전부 도달, 오분류 0.**
- **YMYL**: "고강도 = 고충격이 아니다"를 명시해 관절 제약자를 배제하지 않되, 진단받은 관절·심장 질환은 사전 확인하도록 전 결과에 안내.

### 배선

- `tools/index`·`quiz/index` 카드 추가(New뱃지 + `badge-added:2026-09-01`).
- **홈**: 툴 그리드 추가(홈은 툴 **29개 전량** 노출 구조). 퀴즈 6슬롯 유지 위해 **3개월 노출 0인 `muscle-building-starting-point`를 신규 퀴즈로 교체**.
- `sitemap.xml` 등재 → **112 URL, lastmod 전량, 중복 0**. `llms.txt` 2건.
- 역링크: `sitting-offset-calculator` 사이드바 **+ 본문 문맥링크**(강도 축이 다르다는 연결), `zone-2-vs-hiit-fat-loss` 사이드바, 툴↔퀴즈 상호 → **신규 툴 인바운드 5, 신규 퀴즈 3**.

**검증**: div/a/li/p 밸런스 전 파일 통과, FAQ 본문-스키마 MATCH(7/7, 5/5), JSON-LD 전량 파싱, 인라인 JS `node --check` 통과, 내부링크 깨짐 0, **신규 툴에 `.main-col{min-width:0}` 및 텍스트 스탯 카드용 `.is-text`/모바일 전폭 처리 포함**(08-24 두 차례 UI 사고 재발 방지), 전 사이트 반응형 감사 누락 0건(잔여 1건은 기확인 오탐).

### ⚠️ 다음 세션이 반드시 판단해야 할 것 — 신규 페이지가 6건째 미색인 대기 중

- 현재 **08-24(1차) 2건 + 08-24(3차) 2건 + 이번 2건 = 신규 6건**이 발행돼 있고, 09-01 Coverage 기준 **앞선 2건은 2주째 "발견됨-미색인"**, 나머지는 아직 데이터도 안 잡힘.
- **즉 "포맷 공백 전략"은 아직 단 한 건도 검증되지 않은 상태에서 6건까지 누적됐음.** 이번 세션도 사용자가 신규 발굴을 명시적으로 요청해서 진행한 것이고 전략은 유지하되, **다음 세션에서 Coverage/Bing으로 이 6건의 색인·노출 여부를 반드시 확인하고, 계속 무색인이면 발굴 자체보다 "왜 신규가 색인이 안 되는가"로 초점을 옮길지 판단할 것.**
- 판단 근거가 될 데이터: ①GSC Coverage(발견-미색인 목록에 6건이 어떻게 들어오는지) ②Bing Page 리포트(구글이 안 잡아도 Bing은 잡을 수 있음 — 실제로 Bing은 3주 연속 성장 중).

### 다음 세션 우선순위

1. **신규 6건 색인·노출 확인** (위 참고). Coverage + Bing Page 리포트 양쪽으로 교차.
2. **GSC Performance 요청** — 09-01(1차)에도 못 받았음. 구글 노출 추이 2주째 미확인.
3. Bing 리포트 계속 요청(3주 연속 성장, 실제 클릭 위치를 알려주는 유일한 데이터).
4. Coverage는 가능하면 **요약(색인/미색인 총계)** — 5세션 연속 드릴다운. (※ **구글 서치콘솔** 자료임)
5. GA4 봇 필터(사용자 직접, 7세션 연속 미처리).
6. **⛔ 하지 말 것**(누적): Bing 등록·IndexNow 등 제출 개입, 여성 연령별 체지방 신규 페이지, 비타민D 식품 계산기·독성 단독 페이지, HRR 계산기, 러킹, 스웨트레이트, 디로드 계산기, **수면부채 계산기(이번 기각)**, 퀴즈 0노출 내부링크 재조사, H2 전수대조, DNS·Pages 빌드 가설, boostdomainrating 재론, puppeteer 실물 렌더 검증(컨테이너 불가).

---

## 2026-09-01 세션 (1차, 주간점검) — 주간 점검. 🟢 **Bing 3주 연속 성장(44→241→427 노출, 3→6→11 클릭)** / 🔴 구글 오가닉 2주 연속 GA4 0세션 / 비타민D 계산기 보강 + **연령별 상한 정확성 버그 수정**

**작업 방식**: 사용자가 새 PAT 제공. 자료: **GSC Coverage 드릴다운(09-01)**, Bing 웹마스터 Page/Keyword(08-31), GA4. 대시보드/시각화 금지. Bing은 확정 방침대로 **판단 재료로만** 사용(등록·제출 개입 없음).
**⚠️ 이번엔 GSC Performance(노출/클릭/쿼리) 자료가 없었음.** 구글 쪽 노출 추이는 이번 세션에서 확인 불가 — 판단은 Coverage + GA4로만 했음. 다음 세션엔 Performance도 함께 받을 것.

### 🟢 Bing — 3주 연속 성장, 이제 이 사이트의 실질적 유일 채널

| 주차 | 노출 | 클릭 | CTR | 상위10위 비중 |
|---|---|---|---|---|
| 08-17 | 44 | 3 | 6.82% | — |
| 08-24 | 241 | 6 | 2.49% | 99.5% |
| **08-31** | **427** | **11** | **2.58%** | **99.7%** (11위 밖 단 1노출) |

- **3주간 노출 9.7배, 클릭 3.7배.** 포지션 분포는 1~3위 44노출 / 4~10위 275노출로 여전히 압도적.
- **비타민D 클러스터가 클릭의 55%(11건 중 6건)를 만듦**: `vitamin-d-calculator` 68노출/**5클릭**/6.8위, `how-much-vitamin-d-do-you-need-daily` 49노출/1클릭/6.7위. 08-24(2차)에 "제휴 재개 시 1순위 진입점"으로 적어둔 판단이 한 주 더 강화됨.
- 그 외 클릭: `healthy-body-fat-percentage`(87노출/1클릭), `protein-calculator`(27/1), `calories-burned-by-heart-rate-zone`(10/1/**3.6위** — 08-24 2차에 보강한 페이지, 클릭 유지), `if-calculator`(5/1), `high-protein-snacks-women-over-40`(1노출/1클릭/**2위**).
- 클릭 쿼리 10개 전부 대화형 초롱테일. 신규 등장: **"calculator to avoid vitamin d toxicity"(1위, 클릭)**, "how much rmd vitamin d no sun 39 yr old female pale skin"(7위, 클릭), "custom fastingschedule"(3위, 클릭).

### 🔴 GA4 — 구글 오가닉 2주 연속 0, 봇 비중 여전, 참여시간 악화

- 4주: 활성 471명(직전 420), 이벤트 1,661, **평균 참여시간 9.9초**(직전 13.7초 → 계속 악화).
- **싱가포르 321/471 = 68.2%**(직전 69.3%). Shenzhen 27, San Jose 26 등 데이터센터 출처 여전. **봇 필터 7세션 연속 미설정**(사용자 직접 작업).
- 소스: direct 444(대부분 봇) / **Bing계열 32**(bing 9 + ddg 9 + yahoo 7 + ecosia 4 + cn.bing 2 + ca.yahoo 1) / **AI 6**(copilot 4 + perplexity 2) / **google organic 0(2주 연속)** / ui.awin.com 6.
- **🔵 주목할 신호 — 이탈률**: `Vitamin D Calculator`가 조회 22회에 **이탈률 0.30**. 사이트 다른 페이지는 0.77~1.00 구간이라 **유일하게 정상 참여를 보이는 페이지**임. 2위는 `calories-burned-by-heart-rate-zone` 0.64(08-24 2차 보강 페이지). **이탈률은 지금까지 안 보던 지표인데, Bing 클릭 데이터와 완전히 같은 페이지를 지목함** — 다음 세션부터 보강 대상 선정의 보조 지표로 쓸 것.

### Coverage — 3건 고정, 신규 2건은 아직 데이터 없음

- "발견됨-미색인" **3건**: `quiz/cardio-type.html`(07-25부터 **5주 이상**), `tools/detraining-calculator.html`, `quiz/return-to-training-after-break.html`(08-24 1차 신규, 08-18 발견 이후 2주). 셋 다 최종 크롤 `1970-01-01`.
- **차트가 08-21까지라 08-24(3차) 신규 2건**(`sitting-offset-calculator`, `active-couch-potato`)**은 아직 나타나지 않음.** 다음 세션에 확인.
- 08-24(1차) 신규 2건이 **2주째 발견-미색인**인 것은 주의 신호이지만, `cardio-type`이 5주 걸리고도 아직인 걸 보면 이 사이트의 크롤 예산 자체가 눌려 있는 상태로 보임. **원인 미상 방침 유지, 새 가설 세우지 말 것.**

### 이번 세션에 한 작업 — 보강 1건 (커밋 `5128c08`)

**`tools/vitamin-d-calculator.html` 1,163 → 1,999단어**

- **선정 근거**: 사이트 클릭의 45%를 혼자 만드는 페이지(Bing 68노출/5클릭)이자, GA4 이탈률 0.30으로 유일하게 정상 참여를 보이는 페이지. 08-24(2차)에 확립한 "**Bing에서 이미 클릭 나는 페이지의 결손을 메운다**" 기준을 그대로 적용.
- **Bing 쿼리가 결손을 직접 지목**: ①`calculator to avoid vitamin d toxicity`가 **1위로 클릭까지 발생**했는데 본문의 독성·상한 설명은 **FAQ 1개뿐**이었음 ②`how much micrograms of vitamin d enough per day for adults`(6노출/10위)가 비타민D 최다 무클릭 쿼리인데 **본문에 mcg/microgram 단어가 0회**, μg 기호만 9회 쓰이고 있었음.

**🔴 정확성 버그 수정(콘텐츠보다 우선 처리)**
- 결과의 `Safe upper limit (IU)`가 **4,000으로 하드코딩**돼 있어, **1~8세 나이를 입력해도 성인 상한이 표시**됐음. 실제 UL은 1~3세 2,500 / 4~8세 3,000 IU. 보호자가 아동 나이로 조회하면 실제보다 높은 상한을 보게 되는 상태였고, **"독성 회피" 의도로 유입되는 페이지라 방치 불가**로 판단.
- 연령별 UL 변수를 도입해 표시값과 권장량 캡을 모두 연령 기준으로 교체.
- **검증**: 전 연령(2/5/10/35/75세) × 일조/피부/위도/질환 전체 조합을 돌려 최대 권장값이 해당 연령 UL을 넘지 않음을 확인(2세 2,450/UL2,500, 5세 2,450/UL3,000, 75세 3,250/UL4,000). **수정 전에도 성인은 캡이 걸려 있어 초과는 없었고, 문제는 표시값 오류였음.**

**추가 콘텐츠**
- **IU/mcg 단위 섹션**: 1 mcg = 40 IU 고정 환산, 연령별 RDA·UL을 두 단위 병기한 표. 결과 출력도 `mcg (μg) per day`로 병기.
- **"상한 ≠ 독성 역치" 섹션**: UL 4,000은 안전마진이 들어간 장기 무감독 섭취 천장이지 위해 시작점이 아니라는 구분. 3년 시험에서 400/4,000/10,000 IU의 고칼슘혈증 발생이 0/3/9%였다는 실측치로 규모 감각 제시. **경쟁 문서 대부분이 이 둘을 뭉뚱그리는 지점이라 차별화 축으로 씀.**
- **과잉의 기전 섹션**: 칼슘 흡수 증가 → 고칼슘혈증이 실제 피해 경로, 햇빛으로는 독성 불가능(피부 합성의 자기제한), 칼슘 보충제 병용 시 위험 가중, 지용성이라 회복이 수주~수개월.
- FAQ 7→10개. **YMYL 처리**: 가이드라인 수치와 공개 연구 결과만 제시, 개인 용량 처방 없음, 고칼슘혈증 증상은 진료 대상임을 명시.

### 신규 콘텐츠 후보 — 3건 조사, 전부 기각 (누적 51건 / 성공 2건)

| 후보 | 판정 |
|---|---|
| 비타민D 독성/상한 **단독 페이지** | **기각 → 보강으로 전환.** 아티클을 **Harvard Health·Mayo Clinic·NIH ODS·Drugs.com·UF Health·Wikipedia**가 지배. 익명 브랜드가 YMYL head term으로 경쟁 불가. 다만 우리 계산기가 이미 해당 롱테일에 1위로 클릭받고 있으므로 **신규 페이지 대신 그 페이지에 흡수**(위 작업). 자기잠식도 회피 |
| 스웨트레이트(발한율) 계산기 | **기각 — 양 포맷 포화.** 전용 계산기로 **GSSI(게토레이 스포츠과학연구소) Fluid Loss Calculator**, Precision Hydration, TrainingPeaks, TrueSport, CDC PDF까지 존재. 브랜드 권위도 우리보다 압도적 |
| 디로드(deload) 주간 판단 계산기 | **기각 — 양 포맷 포화.** 아티클 8개+(ProsourceFit·GymPsycho·LoadMuscle·IronLair 등)에 더해 **arvo.guru 전용 deload 계산기**, **anneal.fit 기사 내장 계산기**까지 있음. 08-24 확립 기준(포맷별 공급 분리 집계)으로 봐도 진입로 없음 |

**판단**: 신규 발행 0건. **전략 변경 아님** — 세 건 모두 "포맷별 공급을 따로 세라" 기준을 정상 적용한 결과이고, 계산기 포맷까지 이미 채워져 있어 진입로가 없었음.

### 💰 수익화 관점 우선순위

- 구글 오가닉이 2주 연속 0인 상태에서 **실질 유입은 Bing계열 32세션 + AI 6세션이 전부**. 그래서 이번에도 우선순위는 **"Bing에서 클릭 나는 페이지 보강"**이었고, 그 기준으로 사이트 최대 클릭 페이지를 골랐음.
- **비타민D 클러스터가 클릭의 55%를 만들고 이탈률도 유일하게 정상(0.30)** — 이 사이트에서 실제로 작동하는 유일한 자산임이 3주 연속 데이터로 확인됨. **향후 제휴(보충제 계열)를 재개한다면 진입점은 여기가 명확함.** 다만 제휴 신청 자체는 **트래픽 절대량 때문에 이번에도 보류 권고**(실사용자 월 40~60명 추정, 4세션째 동일).
- 참고: 이번 보강은 수익화 관점에서도 방어적 의미가 있음 — 아동 상한 오표시는 보충제 제휴를 붙였을 때 더 큰 문제가 될 수 있는 항목이었음.

### 다음 세션 우선순위

1. **GSC Performance 자료 요청** — 이번엔 Coverage만 와서 구글 노출 추이를 못 봤음. 08-07부터 시작된 저조가 이어지는지 확인 필요.
2. **Bing 리포트 계속 요청.** 3주 연속 성장 중이고 실제 클릭이 어디서 나는지 알려주는 유일한 데이터.
3. **Coverage에서 08-24(3차) 신규 2건**(`sitting-offset-calculator`, `active-couch-potato`) **등장 여부 확인.** 08-24(1차) 2건과 합쳐 **신규 4건의 색인 진행이 "포맷 공백 전략"의 실질 검증 데이터**.
4. Coverage는 가능하면 **요약(색인/미색인 총계)**으로 — 5세션 연속 드릴다운만 받고 있음. (※ 요약은 **구글 서치콘솔** 쪽 자료임. Bing과 무관.)
5. GA4 봇 필터(사용자 직접, 7세션 연속 미처리).
6. **이탈률을 보강 대상 선정의 보조 지표로 활용할 것** — 이번에 Bing 클릭 데이터와 같은 페이지를 지목했음.
7. 신규 후보 2~3건 조사 계속.
8. **⛔ 하지 말 것**(누적): Bing 등록·IndexNow 등 제출 개입, 여성 연령별 체지방 신규 페이지(2회 기각), 비타민D 식품 계산기(기각)·독성 단독 페이지(기각), HRR 계산기, 러킹, 스웨트레이트, 디로드 계산기, 퀴즈 0노출 내부링크 재조사, H2 전수대조, DNS·Pages 빌드 가설, boostdomainrating 재론, puppeteer 실물 렌더 검증(컨테이너 불가).

---

## 2026-08-24 세션 (3차, 신규 발굴) — ⭐ **좌식시간/활동 상쇄 클러스터 신설.** "아티클 포화 / 툴·퀴즈 공백" 패턴 2번째 적용 사례

**작업 방식**: 사용자가 새 PAT 제공. 신규 발굴 전용 세션(정기점검 아님). 사용자 지시: "키워드 폭넓게 → 리스트 → 경쟁강도 체크 → 강하면 롱테일로 뚫어라 / 왠만하면 클러스터 추가 / GSC 등 우리 데이터 안에서만 보지 말고 **문서수는 적은데 관심(조회수)은 있는 것**". 대시보드/시각화 금지.

### ⭐ 뚫은 자리 — "하루 종일 앉아있는 걸 운동으로 얼마나 상쇄하나"

| 포맷 | 공급 | 판정 |
|---|---|---|
| 아티클 | ScienceAlert 3건, ScienceDaily, LiveScience, Inverse, MedicalNewsToday, Baptist Health, Outside, Yahoo, AIU, WROR 등 **10개+** | 포화 → 아티클 진입 금지 |
| **계산기** | **사실상 0.** LiveLighter(호주 정부 일반 신체활동 도구)와 DeskBreak(휴식알림 앱의 리드젠 도구, 스탠딩·휴식 중심) 2개뿐이며 **둘 다 "내 좌식시간에 맞는 활동 목표"를 산출하지 않음** | **공백 → 진입** |
| **퀴즈** | **0건.** "active couch potato" 아티클은 다수(WEF/Texas Health/FitPro/Excel PT/Borja PT 등)지만 인터랙티브 퀴즈 없음. WEF가 기사 본문에 2문항 자가진단을 넣은 정도 | **공백 → 진입** |

- **08-24(1차) 디트레이닝에 이어 "포맷별 공급 분리 집계" 기준의 2번째 성공 적용.** 이 방법이 재현 가능하다는 신호이므로 다음 세션도 같은 절차를 우선 적용할 것.
- **자기잠식 없음(전수 검증)**: `sedentary|sitting time|desk job|couch potato` grep 결과 히트는 많았으나 **전부 "활동수준 라벨"**(TDEE 계산기의 sedentary 옵션, 체지방 글의 "sedentary person" 비교 문구 등) 또는 부수 언급이었고, **좌식시간 자체를 주제로 다루는 페이지는 0건.** grep 히트 수만 보고 기각했으면 놓쳤을 사례 — **히트가 나와도 "주제인지 라벨인지" 반드시 확인할 것.**
- **기각 후보**: 운동후 심박회복(HRR 1분) — 아티클(Cleveland Clinic·Peloton·Whoop·MedicalNewsToday·MarathonHandbook) + **Omnicalculator 전용 계산기**까지 양쪽 포화이고, 심혈관 위험 예측이라 YMYL 부담도 큼.

### 신규 페이지 2건

**1. `tools/sitting-offset-calculator.html` (1,462단어, FAQ 7) — 클러스터 앵커**
- 모델: `target = 22 + max(0, 좌식h − 4) × 2.6`, 45분 상한. WHO 2020 하한(150분/주 ≈ 22분/일)을 바닥으로 좌식시간에 따라 상향. 4h→22, 8h→32, 10h→38, 12h→43분.
- 근거: 2020년 가속도계 메타분석(9개 연구/44,370명)의 **30~40분/일** 구간, 2024년 직업적 좌식 연구의 **+15~30분/일**.
- **차별화 축 2개(이게 이 페이지의 존재 이유)**:
  1. **2016년 연구의 60~75분과 2020년의 30~40분이 충돌하는 이유를 측정방식 차이(자가보고 vs 기기측정)로 설명.** 경쟁 아티클들이 두 숫자를 근거 없이 뒤섞어 쓰는 지점이라, 여기를 정리해준 것 자체가 차별점. 본 계산기는 기기측정 기준임을 명시.
  2. **"좌식행동 ≠ 신체활동 부족"을 분리해 출력을 2개로 나눔.** 분 단위 목표는 비활동을, 30분마다 2~5분 휴식 플랜은 좌식시간을 각각 다룸. 한쪽을 채워도 다른 쪽은 안 닫힌다는 게 이 주제의 핵심 인사이트.
- 5개 패턴 분류 출력(핀란드 연구 taxonomy 반영). 1,131개 조합 시뮬레이션으로 전부 도달 확인.
- **🟢 표-로직 정합성 사전 검증**: 본문 표 5행을 실제 함수로 전수 재계산해 **전 행 일치**(22/27/32/38/43분, 주간 154/189/224/266/301 → 표기 150/190/225/265/300). 경계값 0h/18h/20h 상한 동작도 확인. **08-17·08-24(1차)의 사후 발견과 달리 2세션 연속 사전 차단 성공 — 절차가 정착됨.**

**2. `quiz/active-couch-potato.html` (1,058단어, FAQ 5)**
- **결과 taxonomy를 임의 창작하지 않고 핀란드 가속도계 연구(약 3,700명)가 실제 도출한 4개 프로필**(active couch potato / sedentary light mover / sedentary exerciser / mover)을 그대로 사용. 근거 있는 분류라는 게 경쟁 대비 핵심 차별점이며, "약 1/3이 ACP, 최적 그룹(mover)은 17%뿐"이라는 실측 수치도 본문에 활용.
- 5문항 → 5개 결과. **1,024개 조합 전수 시뮬레이션: 전부 도달, 오분류 0**(ACP 128 / SedExerciser 128 / Mover 256 / SedLightMover 256 / LowSitUnder 256).
- **YMYL 관리**: 전 결과에 "움직임 패턴 설명이지 진단 아님" 명시, 장기 비활동자·심혈관/대사질환자는 의료진 상담 안내. **개인 사망위험 예측 표현은 전면 배제**(원 연구가 집단 평균이라는 점을 본문에 명시).

### 배선

- `tools/index` / `quiz/index` 카드 추가(New뱃지 + `<!-- badge-added:2026-08-24 -->`).
- **홈**: 툴 그리드 추가(홈은 툴 **28개 전량** 노출 구조). 퀴즈 미리보기 6슬롯 유지를 위해 **3개월 노출 0인 `zone-2-fitness-level`을 신규 퀴즈로 교체**(08-24 1차의 `recovery-type` 교체와 같은 원칙).
- `sitemap.xml` 등재(툴 0.8 / 퀴즈 0.7) + 인덱스 3건 lastmod 갱신 → **110 URL 전부 lastmod, 중복 0, XML 유효**.
- `llms.txt` 2건 등재.
- **역링크**: `calorie-calculator` / `active-recovery-calculator` 사이드바, `10000-steps-a-day`는 사이드바 + **"Beyond steps" 섹션 본문 문맥링크**(스텝 수가 하루 나머지 시간을 설명하지 못한다는 연결), `calories-burned-walking` 사이드바. 툴↔퀴즈 상호링크 포함 → **신규 툴 인바운드 7개, 신규 퀴즈 3개**.

### 🔧 같은 날 후속 — 모바일 결과카드 UI 수정 (커밋 `cc84b45`)

사용자가 `sitting-offset-calculator` 모바일 스크린샷 제보. 3번째 스탯 카드의 `Sedentary, under target`이 3줄로 접혀 카드가 비정상적으로 길어짐(PC는 정상).

- **원인 2가지가 겹침**: ①`.stat-val`이 **숫자 표시용**으로 Fraunces 1.5rem인데 이 카드만 값이 문구(최대 23자)라 큰 세리프로 렌더됨 ②768px 이하에서 `.stats-grid`가 2열이 되는데 카드가 3개라 **마지막 카드가 혼자 반폭으로 남음**. 숫자였으면 문제없었을 조합.
- **수정**: `.stat-val.is-text`(DM Sans .95rem) 추가 + 768px 이하에서 마지막 카드 `grid-column:1/-1` 전폭 처리. CSS 3줄, 마크업·로직 변경 없음.
- **범위 판단**: 같은 3카드 구조인 `active-recovery`/`detraining`/`maf`/`zone-2`도 확인했으나 3번째 값이 전부 숫자이거나 한 단어라 반폭에서도 정상 → **건드리지 않음.**
- **⚠️ 규칙으로 등재**: **스탯 카드에 문구(텍스트) 값을 넣을 땐 `.stat-val`을 그대로 쓰지 말 것.** 이 클래스는 숫자 전용 서체이며, 카드 3개 + 모바일 2열 조합에서 마지막 카드가 반폭이 되므로 긴 문구는 반드시 전폭 처리와 축소 서체를 함께 적용해야 함. 신규 툴에서 텍스트형 결과 카드를 쓸 때마다 확인할 것.

**검증**: div/a/li/p 밸런스 전 파일 통과, FAQ 본문-스키마 MATCH(7/7, 5/5), JSON-LD 전량 파싱, 인라인 JS `node --check` 통과, 사이트 전체 내부링크 깨짐 0, **신규 툴에 `.main-col{min-width:0}` 포함**(08-24 반응형 사고 재발 방지), 전 디렉토리 반응형 재감사 누락 0건(잔여 1건은 기확인 오탐).

### 다음 세션 참고

1. **신규 2건 색인·노출 추적.** 08-24(1차) 디트레이닝 2건이 아직 발견-미색인 상태이므로, **이번 2건까지 합쳐 신규 4건의 색인 여부가 "포맷 공백 전략"의 실질 검증 데이터가 됨.** 구글 색인이 안 붙어도 Bing 쪽에서 먼저 잡힐 가능성이 있으니 Bing 리포트로 교차 확인할 것.
2. **좌식 클러스터는 3번째 페이지 여지가 있음** — 이번엔 "스탠딩 데스크가 실제로 도움이 되는가"를 툴/퀴즈 양쪽에서 검증하지 않았음. 다만 **무리해서 늘리지 말 것**: 앵커 2개로 충분한지 색인 결과를 보고 판단.
3. **발굴 절차가 2회 연속 통했으므로 그대로 유지**: ①주제 후보 → ②아티클 수 / 전용 툴 수 / 퀴즈 수를 **따로** 집계 → ③툴·퀴즈가 비어 있으면 진입 → ④기존 파일 grep 시 **"주제인지 활동수준 라벨인지" 구분** → ⑤표·수치는 실제 함수로 사전 재계산 → ⑥분기 로직 전수 시뮬레이션.
4. **⛔ 하지 말 것**(누적): Bing 등록·IndexNow 등 제출 개입, 여성 연령별 체지방 신규 페이지(2회 기각), HRR 계산기(이번 기각), 러킹, 퀴즈 0노출 내부링크 재조사, H2 전수대조, DNS·Pages 빌드 가설, boostdomainrating 재론, puppeteer 실물 렌더 검증(컨테이너 불가).

---

## 2026-08-24 세션 (2차, 같은 날 주간점검) — 주간 작업을 앞당겨 진행. 🔴 **구글 노출붕괴 15일째 지속 + GA4 구글 오가닉 0세션** / 🟢 **Bing 노출 5.5배 성장, 클릭 수에서 구글과 동률**

**작업 방식**: 사용자가 새 PAT 제공. 자료: GSC Performance + Coverage(08-11/17/24 3주치), **Bing 웹마스터 Page/Keyword 리포트**, GA4(07-20~08-16 기준 다음 창). 대시보드/시각화 금지. Bing은 08-11 확정대로 **판단 재료로만** 사용(등록·제출 개입 없음).

### 🔴 구글: 붕괴가 회복되지 않고 15일째 지속. GA4에서 구글 오가닉이 아예 사라짐

- 일별 노출: 08-07부터 `1/0/1/4/4/4/7/8/5/5/7/5/6/4/6` — **15일 연속 한 자릿수**. 이전 붕괴(07-23~28, 08-02~04)는 2~6일 뒤 120회/일로 자동 복구됐으나 **이번엔 복구 신호 전무**. 08-06이 마지막 정상일(89회)이었음.
- **GA4 세션 소스에서 `google/organic`이 목록에서 통째로 사라짐**(직전 4세션 → 0). 지난 세션까지는 소수라도 잡혔는데 이번엔 아예 없음. GSC 노출 붕괴와 정합.
- 3주 누적 추이(3개월 롤링 창): 노출 11,585 → 11,277 → **10,786**, 클릭은 3주 내내 **6회 고정**. 붕괴일이 창에 들어오면서 노출만 깎이는 중.
- 쿼리 포지션 분포 **4개월째 미동**: 51~100위 95.9% → 95.9% → **95.5%**. 1~10위는 0.2%로 고정.
- 페이지별 노출은 특정 페이지가 아니라 **전 페이지에서 고르게 감소**(body-fat-calculator −75, calories-burned-walking −69, zone-2-beginners −65 등). 사이트 레벨 요인이라는 기존 해석과 일관.
- **원인은 여전히 미상.** 08-17 세션의 링크스팸 단정은 이미 철회됐고, 인프라 가설(DNS·Pages 빌드)도 전부 소진됨. **새 가설을 세우기보다 추이만 볼 것**(08-17 확정 방침 유지).

### 🟢 Bing: 노출 5.5배 성장, 클릭 수에서 구글과 동률이 됨

| | 노출 | 클릭 | CTR | 포지션 |
|---|---|---|---|---|
| **Bing (08-24)** | **241** | **6** | **2.49%** | 상위10위 내 **99.5%** |
| Bing (08-17) | 44 | 3 | 6.82% | — |
| Google (3개월) | 10,786 | 6 | 0.06% | 51~100위 95.5% |

- **1주 만에 노출 44 → 241(5.5배), 클릭 3 → 6.** 이제 **Bing이 구글과 같은 클릭 수를 구글 노출의 2%로 만들어내고 있음.**
- 포지션 분포: 1~3위 34노출 / 4~10위 153노출 / **11위 이하 단 1노출**. 구글과 정반대.
- Bing 상위 페이지: `healthy-body-fat-percentage`(55노출/1클릭/7.2위), `vitamin-d-calculator`(35/**3클릭**/7.4위), `how-much-vitamin-d-do-you-need-daily`(21/5.2위), `heart-rate-zone`(17/7.1위), `active-recovery-calculator`(10/**2.6위**), `quiz/metabolism-type`(1/**1.0위**).
- **클릭 발생 쿼리 5개 전부 초롱테일 자연어**: "how to calculate how much vitamin d i need give me the calculator"(2클릭, 1위), "vitamin d per day foods calculator"(1위), "what is a truly healthy body fat percentage for a 55 year old woman"(5위), "good protein snacks for adult women in their 40s"(2위), **"what calories does zone 1 burn per day"(3위)**.
- **해석**: 08-17에 확인한 "콘텐츠 품질이 아니라 구글의 도메인 평가가 병목"이 한 주 더 강하게 재확인됨. 같은 콘텐츠가 한쪽에선 상위 10위 99.5%, 다른 쪽에선 51위 밖 95.5%. **롱테일 전략도 유효함이 재확인**(클릭 5건 전부 대화형 롱테일). 다만 절대 볼륨은 여전히 작음.
- **Bing 리포트는 이제 이 프로젝트에서 가장 실용적인 콘텐츠 신호원임.** 어떤 쿼리로 클릭이 실제로 발생하는지 알려주는 유일한 데이터라, **다음 세션도 반드시 요청할 것.**

### Coverage — 신규 2건이 며칠 만에 발견됨(sitemap lastmod 효과 정황)

- "발견됨-현재 색인이 생성되지 않음" 3건: `quiz/cardio-type.html`(기존), **`tools/detraining-calculator.html`**, **`quiz/return-to-training-after-break.html`**(08-24 신규 2건).
- 차트상 08-17까지 1건 → **08-18에 3건으로 점프**. 즉 **신규 페이지가 발행 직후 며칠 만에 구글에 발견됨**. 08-11에 넣은 sitemap lastmod가 발견 단계에서는 작동하고 있다는 정황.
- 다만 셋 다 최종 크롤 `1970-01-01`(미크롤). 신규 페이지가 발견→미색인 상태인 건 초기엔 정상이므로 **3~4주 더 관찰 후 판단.** `cardio-type`은 07-25부터 한 달 넘게 같은 상태.
- **참고로 형제 사이트 `cookingcalcs.com`도 발견-미색인 11건이 몇 주째 고정**(사용자가 함께 첨부). 이 계정 산하 사이트 전반의 공통 현상일 가능성이 있으나, **단정하지 말 것** — 이번엔 커버리지만 받았고 노출 그래프는 못 봤음.

### GA4 — 봇 비중 오히려 악화

- 4주: 활성 420명(직전 379), 이벤트 1,586, 평균 참여시간 **13.7초**(직전 15.7초 → 악화).
- **싱가포르 291/420 = 69.3%**(직전 63.9% → **악화**). San Jose 21, Shenzhen 18, Guangzhou 8 등 데이터센터 출처 여전. **GA4 봇 필터 6세션 연속 미설정**(사용자 직접 작업).
- 소스: direct 392(대부분 봇) / **Bing계열 28**(bing 8 + ddg 8 + yahoo 7 + ecosia 3 + cn.bing 2) / **AI 어시스턴트 7**(copilot 5 + perplexity 2) / **google organic 0** / ahrefs 3(크롤러).
- 조회 상위 페이지는 Bing 데이터와 일치: 홈 42, **Vitamin D Calculator 23**, MAF 180 20, Zone 2 beginners 20.

### 이번 세션에 한 작업 — 보강 1건 (커밋 `866a5c5`)

**`blog/calories-burned-by-heart-rate-zone.html` 1,055 → 1,895단어**

- **선정 근거가 이번 세션의 핵심**: 이 페이지는 Bing 평균순위 **3.4위**이고, **실제 클릭이 난 5개 쿼리 중 하나("what calories does zone 1 burn per day", 3위)의 착지 페이지**임. 그런데 본문의 Zone 1 언급은 표 한 줄 포함 **단 2회**, `per day`/`daily` 프레이밍은 **0건**이었음 — **클릭을 받은 질문에 정작 답이 없는 상태.** 동시에 1,055단어로 블로그 품질기준(1,500) 미달.
- 추가: ①Zone 1 세션별 소모(30/45/60분 = 150/220/300 kcal) ②**총소모 vs 순소모 구분** — 심박 공식은 gross라 휴식대사 포함, 차감 시 120/175/240 kcal. 고강도 존에선 무시할 오차지만 Zone 1에선 총량의 1/5이라 "Zone 1이 체감보다 적게 잡히는 이유"를 설명하는 차별화 축으로 씀 ③**"per day"의 두 의미 분리** — 의도적 이지세션(순증 120~240) vs 워치가 집계하는 하루 총합(일상활동이 같은 심박대라 휴식대사 이중계산) ④Keytel 유효구간 90~150bpm과 Zone 1 하한 93bpm이 겹친다는 점 ⑤FAQ 4→7개.
- **🟢 수치 검증 — 3세션 연속 사고를 이번엔 사전 차단함**: 삽입 전 기존 표를 역산해 **여성 Keytel식 + 존 중앙값 기준**임을 특정(계산 4.53/6.51/8.49/10.46/12.44 → 표기 5/7/9/11/13, 올림 일관)하고 본문 수치를 같은 기준에 맞춤. 휴식대사 1 kcal/min도 Mifflin으로 교차검증. **08-17·08-24(1차) 두 번의 표-로직 불일치와 달리 이번엔 사후 발견이 아니라 사전 검증으로 처리됨 — 이 절차를 계속 유지할 것.**

### 신규 콘텐츠 후보 — 2건 조사, 전부 기각 (누적 46건 / 성공 2건)

Bing 클릭 데이터에서 수요가 확인된 두 축을 우선 조사함:

| 후보 | 근거 | 판정 |
|---|---|---|
| 여성 연령별 체지방률 페이지 | Bing에서 **7개+ 쿼리 군집**("55 year old woman", "woman aged 56", "61 yrar old woman", "women 25 years old athletic" 등), `healthy-body-fat-percentage`가 Bing 1위 노출 페이지 | **기각 — 자기잠식.** 기존 페이지가 3,023단어에 `ranges by sex and age` / `how body fat changes with age` 전용 H2를 이미 보유. 08-17에도 같은 이유로 기각한 바 있음(재조사 금지) |
| 비타민D 식품 섭취량 계산기 | Bing 클릭 6건 중 **3건이 비타민D**, "vitamin d per day foods calculator"가 1위 클릭 | **기각 — 전용 계산기 15개+ 포화.** missvickie(연령·식품·햇빛·보충제 통합, 우리 구상과 동일), best-calculators, thecalcs, codingace, monocalc, healthcalculators.xyz, vitamin-calculator.com, vovkey, macroandmeals, vitagene, omegaquant, grassrootshealth 등. **`nutritioncalcs.com`이 또 등장(4번째)** — 상시 경계 대상 재확인 |

- **판단**: 신규 발행 0건. **전략 변경 아님** — 두 후보 모두 08-24(1차)에 확립한 "포맷별 공급을 따로 세라" 기준을 적용했고, 체지방은 우리 쪽 공급이 이미 충분(자기잠식), 비타민D는 상대 쪽 공급이 포화라 **양쪽 다 진입로가 없었음**. 사용자가 명시한 "진짜 할 게 없으면 안 하는 게 맞다" 적용.

### 반응형 재감사 — 지난주 누락 범위 보완, 전 사이트 클린

- 08-24(1차) 반응형 감사가 `tools/` 한정이었으므로 **blog/·quiz/·루트까지 확대 재감사**함.
- **블로그 템플릿은 컨테이너 클래스명이 다름**: `.article-wrap{grid-template-columns:1fr 300px}` + `.article-main`. 툴은 `.page-wrap{1fr 340px}` + `.main-col`. **지난주 grep(`main-col{min-width:0`)에 블로그가 안 잡힌 이유가 이것** — 결함이 아니라 탐지 누락이었고, 확인 결과 블로그는 `.article-main{min-width:0}`을 정상 보유.
- 전 사이트 스캔 결과 **누락 0건**. `intermittent-fasting-for-women-over-40`이 1건 걸렸으나 `min-width:180px`가 `flex-wrap:wrap`된 flex 아이템이고 표도 없어 **오탐**.
- **⚠️ 다음 감사 시 주의**: 클래스명이 디렉토리별로 다르므로 `grep main-col`만으로 감사하면 블로그·퀴즈를 통째로 놓침. `grid-template-columns:1fr \d+px` 정규식으로 컨테이너를 먼저 찾고, 자식 후보(`main-col|article-main|content-col|quiz-main`)를 함께 볼 것.

### 💰 수익화 관점 우선순위

- 구글이 사실상 0인 상태에서 **실질 유입 채널은 Bing계열 28세션 + AI 어시스턴트 7세션이 전부**(GA4 봇 제외 추정 실사용자 월 40~60명, 3세션째 동일).
- **따라서 이번 세션 우선순위 판단은 "Bing에서 이미 클릭이 나는 쿼리의 착지 페이지를 보강한다"였고, 그게 이번 보강 1건의 선정 기준이었음.** 신규 페이지는 구글에서 색인조차 안 되는 상황이라 한계수익이 0에 가깝고, Bing에서 3위로 클릭까지 받는 페이지의 결손을 메우는 게 훨씬 확실한 기대값.
- **제휴(iHerb 재신청·신규 신청)는 이번에도 보류 권고.** 사유는 변함없이 **트래픽 절대량**. 다만 참고로 **비타민D 클러스터가 Bing 클릭의 절반(3/6)을 만들고 있어, 향후 제휴를 재개한다면 이 클러스터가 가장 유력한 진입점**임(보충제 제휴와 직결). 트래픽이 회복되면 여기부터 볼 것.

### 다음 세션 우선순위

1. **Bing 리포트 반드시 재요청** — 현재 이 프로젝트에서 실제 클릭이 어디서 나는지 알려주는 유일한 데이터. 노출 5.5배 성장이 이어지는지, 클릭 쿼리가 늘어나는지가 다음 보강 대상을 결정함.
2. 08-24(1차) 신규 2건(`detraining-calculator`, `return-to-training-after-break`) **색인 여부 확인**. 발견은 됐고 크롤 대기 중. **포맷 공백 가설의 첫 검증 사례라 결과가 향후 발굴 전략을 좌우함**(08-24(1차) 기록 참고).
3. 구글 노출 추이만 관찰. **원인 미상 유지, 새 가설 금지.**
4. Coverage는 **요약(색인/미색인 총계)**으로 — 4세션 연속 드릴다운만 받고 있음.
5. GA4 봇 필터(사용자 직접, 6세션 연속 미처리).
6. 신규 후보 2~3건 조사 계속. **Bing 클릭 쿼리를 후보 발굴의 1차 소스로 쓸 것** — 이번에 Zone 1 결손을 찾아낸 방식이 그것.
7. (낮음) 기존 퀴즈 20개 `updateProgress` 진행바 NaN 버그 확인.
8. **⛔ 하지 말 것**: Bing 등록·IndexNow 등 제출 개입, 여성 연령별 체지방 신규 페이지(2회 기각), 퀴즈 0노출 내부링크 재조사, H2 전수대조, DNS·Pages 빌드 가설, boostdomainrating 재론, puppeteer 실물 렌더 검증(컨테이너에서 불가).

---

## 2026-08-24 세션 (1차, 신규 발굴) — 신규 클러스터 발굴(사용자 지시). ⭐ **디트레이닝/훈련 복귀 클러스터 신설** — 아티클은 포화지만 계산기·퀴즈 포맷이 영어권에 비어 있는 자리를 찾음

**작업 방식**: 사용자가 새 PAT 제공. 이번 세션은 정기점검이 아니라 **신규 발굴 전용**. 사용자 지시: "키워드 다양하게 폭넓게 → 리스트 → 경쟁강도 체크 → 강하면 롱테일로 뚫어라 / 왠만하면 클러스터를 추가할 수 있도록 / GSC 등 우리 데이터 안에서만 보지 말고 구글·네이버·레딧 등 다양한 곳에서 **문서수는 적은데 관심(조회수)은 있는 것**을 찾아라". 대시보드/시각화 금지.

### ⭐ 이번에 뚫은 자리 — "아티클 포화 / 툴·퀴즈 공백" 패턴

**디트레이닝(detraining, 훈련 중단 시 체력 손실)** 주제를 신규 클러스터로 신설함. 선정 논리가 이번 세션의 핵심이라 그대로 남김:

| | 공급(문서 수) | 판정 |
|---|---|---|
| 아티클 | Garmin, Firstbeat, Wahoo, TrainingPeaks, BikeRadar, InBody, StrongerByScience, RunnersConnect, MarathonHandbook, PureGym, MyFitnessPal, BarBend 등 **15개+** | 포화 → **아티클로는 진입 금지** |
| **계산기** | **영어권 전용 페이지 0건.** magicfit.fr(프랑스어), runnersconnect의 이메일 수집용 다운로드 파일, thunderrunners의 마라톤 한정 인라인 위젯이 전부 | **공백 → 진입** |
| **퀴즈** | **0건.** "return to training readiness" 검색 결과가 배변훈련·커리어·원격근무 퀴즈로 채워짐 | **공백 → 진입** |

- **아티클 15개+가 존재한다는 사실 자체가 수요의 증거**이고, 같은 수요를 받아낼 계산기가 없다는 게 공급 공백임. 사용자가 요구한 "문서수 적고 조회수 많은" 조건의 교과서적 사례라 판단.
- **이 사이트의 실측 패턴과 정확히 일치**: 08-11 세션에서 퀴즈 중앙값 순위 **14.0** vs 툴 58.3 / 블로그 47.9로, **"경쟁사가 안 만드는 포맷"이 이 사이트의 유일한 해자**임을 확인한 바 있음. 이번 건은 그 해자를 의도적으로 재현한 첫 사례.
- **자기잠식 없음(전수 검증)**: `detrain|lose fitness|time off|comeback` 전수 grep 결과 기존 블로그 56 / 툴 27 / 퀴즈 21 전체에서 **언급 0건**. 완전 신규 영역. `how-to-improve-vo2-max`(향상)와는 방향이 반대라 카니발라이제이션 아님.
- **기각한 후보**: 러킹 칼로리 계산기 — GORUCK(브랜드 권위) 포함 **전용 계산기 12개+**로 포화(hyperwear, rukstr, shreddeddad, megacalculatorhub, megatoolcalculator, creativewidgets, rucking.pro 2종, walkcalculator, getrucky 등).

### 신규 페이지 2건

**1. `tools/detraining-calculator.html` (1,497단어, FAQ 8) — 클러스터 앵커**
- Coyle / Mujika &amp; Padilla 데이터를 **감쇠곡선** `plateau*(1-exp(-k*w))`(plateau 22, k 0.105)로 모델링. 선형이 아니라 초기 급락 후 완만해지는 형태 — "2주 오프가 그다음 2주보다 훨씬 비싸다"는 게 이 주제의 핵심 인사이트라 곡선 형태 자체가 콘텐츠임.
- 수정자 3종: **훈련경력**(highly 1.18x / rec 0.82x), **중단 중 활동**(경도 0.72x / 주1~2회 감량훈련 0.32x), **50세 초과 연령 보정**(최대 +15%).
- **근력은 별도 로직**(3주까지 평탄 → 이후 1.1%/주)으로 분리. 유산소와 근력이 다른 시계열로 감소한다는 게 이 주제에서 가장 유용한 사실이고, 경쟁 아티클들이 뭉뚱그리는 지점이라 차별화 축으로 씀.
- **⚠️ 이번에도 표-로직 불일치를 잡음(2세션 연속 같은 유형)**: 초안 표의 근력 유지율이 계산 로직과 어긋났음(4/8/12주에서 표 95/88/82% vs 계산 99/97/94%). 감소율을 문헌에 맞게 0.62 → **1.1%/주**로 조정하고 표를 계산 출력값(99/95/90%)으로 동기화. VO2 열은 이미 일치했음(계산 2.2/4.2/7.5/12.5/15.8 vs 표 2/4/8/13/16).
  → **08-17 세션(calories 툴 표 8개 값 전부 오류)에 이어 두 번째. 계산기 페이지에 예시 수치를 넣을 땐 반드시 그 페이지의 공식으로 직접 계산해 검증할 것.** 이건 이제 우연이 아니라 반복 패턴임.
- 극단값 검증: 0.5주 / 52주 / 260주(캡 동작) / 감량훈련 조합 전부 정상 범위.

**2. `quiz/return-to-training-after-break.html` (1,186단어, FAQ 5)**
- **각도 설계가 핵심**: "얼마나 잃었나"가 아니라 **"어떻게 돌아갈 것인가"**로 잡음. 전자는 바꿀 수 없고 후자는 바꿀 수 있어서, 아티클 15개가 전부 전자만 다루는 공백을 노린 것.
- 5문항(기간 / 원인 / 중단 중 활동 / 훈련경력 / 현재 몸상태) → 4개 결과(부상·잔존통증 / 그냥 재개 / 유산소 재구축 / 훈련 블록).
- **부상·잔존 통증은 별도 분기로 분리**해 타임라인 대신 **의료 클리어런스 우선**을 안내. YMYL 리스크 관리 목적이며, 조합의 43.8%가 이 분기로 빠지도록 보수적으로 설계함.
- **1,024개 조합 전수 시뮬레이션**으로 4개 결과 전부 도달 가능 + 오분류 0 확인(부상 448 / 재개 216 / 재구축 216 / 블록 144).
- **⚠️ 템플릿 버그 발견**: `quiz/creatine-right-for-you.html`의 `updateProgress(q,total)`가 `nextQ`에서 인자 1개로만 호출돼 **진행바 폭이 NaN%가 되는 버그**가 있음. 이번 신규 퀴즈는 `TOTAL` 상수로 고쳐서 작성했으나, **기존 퀴즈 20개에 같은 버그가 있는지는 확인 안 함 — 다음 세션 점검 후보**(다만 진행바 폭만 영향, 기능·색인엔 무관하므로 우선순위 낮음).

### 배선 (신규 페이지 체크리스트 전항 수행)

- `tools/index.html` / `quiz/index.html` 카드 추가(New뱃지 + `<!-- badge-added:2026-08-24 -->`).
- **홈**: 툴 그리드에 추가 — **홈은 툴 27개를 전량 노출하는 구조**라 신규 툴은 반드시 반영해야 함(handover의 "미리보기 3슬롯" 서술은 퀴즈·블로그에만 해당). 퀴즈 미리보기는 6슬롯 고정이라, **3개월 노출 0인 `recovery-type`을 신규 퀴즈로 교체**.
- `sitemap.xml` 등재(툴 0.8 / 퀴즈 0.7) + 수정한 인덱스 3건 lastmod 갱신 → **108 URL 전부 lastmod 보유, 중복 0, XML 유효**.
- `llms.txt` 2건 등재.
- **역링크**: `vo2-max-calculator` / `zone-2-calculator` / `active-recovery-calculator` 사이드바 + `how-to-improve-vo2-max` **본문 문맥링크**. 툴↔퀴즈 상호링크까지 배선해 **신규 툴 인바운드 5개, 신규 퀴즈 4개** 확보.
  - 단 08-11 세션 발견 참고: **인바운드 수와 노출은 역상관이었음**(0노출 퀴즈가 인바운드 10개, 최고성과 퀴즈가 1개). 링크는 고립 방지 목적이지 순위 기대 근거가 아님.

### 🔴 같은 날 후속 — 반응형 깨짐 수정 (커밋 `ccd142c`)

사용자가 `/tools/detraining-calculator.html` 좁은 화면 스크린샷 제보. 본문 전체가 뷰포트보다 넓어져 우측이 잘림(제목·설명·입력폼까지 전부).

- **원인**: `.page-wrap`이 CSS Grid(`1fr 340px`)인데 **그리드 아이템의 기본값이 `min-width:auto`**라, 자식 중 `min-width:520px`인 표가 있으면 컬럼이 그 아래로 축소되지 못하고 그리드 전체를 밀어냄. **`.table-wrapper`의 `overflow-x:auto`만으로는 소용없음** — 스크롤 컨테이너가 되기 전에 그리드 아이템 자체가 안 줄어들기 때문. 768px 미디어쿼리로 1단 전환이 돼도 컬럼 폭이 520px+로 고정돼 증상이 그대로 남음.
- **기존 정상 툴 페이지 15개에는 전부 `.main-col{min-width:0;}`이 있었음.** 신규 생성분과, 나중에 표를 추가한 페이지에서만 이 한 줄이 누락된 것.
- **수정**: 표 보유 툴 페이지 전수 감사 후 누락 4건에 규칙 추가 — `detraining-calculator`(08-24 신규), **`calories-burned-by-heart-rate`(08-17에 표를 추가하면서 같은 결함을 유입시킴)**, `ffmi-calculator`, `lean-body-mass-calculator`(둘 다 기존 잠재 결함). 재감사 결과 표 보유 툴 19개 전부 보유 상태.
- **⚠️ 규칙으로 등재**: **`.table-wrapper`를 새로 넣는 페이지는 `.main-col{min-width:0;}` 보유 여부를 반드시 같이 확인할 것.** 표 반응형 패턴은 이 두 줄이 세트이며, wrapper만 넣으면 오히려 페이지 전체가 깨짐. 08-17에 이 실수를 하고 08-24에 같은 실수를 반복했음.
- **검증 한계**: 실제 브라우저 렌더 검증을 시도했으나 Chrome 바이너리 다운로드가 네트워크 허용목록에 막힘(`storage.googleapis.com` → 403 `host_not_allowed`). **이 컨테이너에서는 puppeteer 실물 검증이 불가하므로 다음 세션도 시도하지 말 것.** 대신 고정폭 전수 스캔, 래핑 누락 표 0건 확인, 28자 초과 무공백 토큰 0건, grid-template-columns 축소 경로 확인으로 대체함.

**검증**: div/a/li/ul 밸런스 전 파일 통과, FAQ 본문-스키마 MATCH(8/8, 5/5), JSON-LD 전량 파싱, 인라인 JS `node --check` 전 파일 통과, **사이트 전체 내부링크 스캔 깨진 링크 0**(잔여 5건은 `assets/partials/`의 `{{BASE}}` 템플릿 변수로 기존 정상).

### 다음 세션 우선순위

1. **신규 2건 색인·노출 추적** — 3~4주 뒤가 판단 시점. 특히 **이 클러스터는 "포맷 공백" 가설의 첫 의도적 검증 사례**라, 결과가 향후 발굴 전략을 좌우함. 잘 되면 같은 패턴(아티클 포화 + 툴/퀴즈 공백)을 반복 적용, 안 되면 포맷 해자 가설 자체를 재검토.
2. 08-07 이후 계속된 노출 저조가 회복되는지 추적. **원인 미상 상태 유지, 새 가설 세우기보다 추이만 볼 것.**
3. Coverage는 **드릴다운 말고 요약(색인/미색인 총계)** — 3세션 연속 못 받았음.
4. GA4 봇 필터(사용자 직접, 6세션 연속 미처리).
5. 신규 후보 조사 계속. **이번에 확인된 유효 패턴**: 특정 주제에서 아티클 공급과 툴/퀴즈 공급을 **따로 세어** 비율을 볼 것. 아티클 다수 = 수요 증거, 툴 0 = 진입로.
6. (낮음) 기존 퀴즈 20개의 `updateProgress` 진행바 NaN 버그 여부 확인.
7. (낮음) 블로그·퀴즈 쪽에도 `table-wrapper`가 있는데 그리드 레이아웃을 쓰는 페이지가 있는지 확인(이번엔 툴 디렉토리만 감사했음).
7. **⛔ 하지 말 것**: Bing 등록·IndexNow 등 Bing 대상 제출 개입, 퀴즈 0노출 내부링크 재조사, H2 전수대조, DNS 재확인, GitHub Pages 빌드실패 가설, boostdomainrating 관련 재론(08-17 참고).

---

## 2026-08-17 세션 — 일요일 정기점검. Bing 데이터 최초 확보로 "콘텐츠가 아니라 도메인 신뢰가 병목"임이 실증됨 + 홈 외부배지 rel 정리, 툴 1건 보강

**작업 방식**: 사용자가 새 PAT 제공. 대시보드/시각화 금지. 신규 콘텐츠는 ①기존 파일 중복확인 ②웹서치 경쟁강도 ③롱테일 ④수익화 우선순위. 공격적 확장 전략 유지(단 "진짜 할 게 없으면 안 하는 게 맞다").
**분석 자료**: GSC Performance + Coverage Drilldown(08-17), GA4 보고서 개요(07-20~08-16), **Bing 웹마스터도구 PageTraffic + Keyword 리포트(08-17) — 이 프로젝트 최초**.

### 노출붕괴 관련 — 이번 세션에 정리한 사실과, 확정되지 않은 추측의 구분

**⚠️ 먼저 읽을 것 — 이 절의 인과관계는 확정된 것이 아님.** 세션 중 Claude가 "원인을 특정했다"고 단정적으로 보고했으나 **이는 시점 상관만 보고 내린 과잉 결론이었고, 사용자가 지적해 철회함.** 아래는 (a)확인된 사실과 (b)Claude의 미확정 추측을 구분해 남긴 것이며, **(b)를 확정된 원인으로 읽고 후속 작업을 설계하지 말 것.**

**사용자 확인 사항**: boostdomainrating.com은 **백링크 확보 목적으로 사용자가 직접 등록한 것**임. 세션 중 Claude가 "유료 구매 여부"를 물은 것은 넘겨짚은 것으로, 사용자가 명확히 부정함. **이 각도로 다시 질문하거나 disavow를 권고하지 말 것.** 또한 사용자는 **이 배지에 rel 처리를 하지 않아도 상관없다는 입장**을 밝힘.

- 홈 `index.html` 하단 외부 배지 9개 중 **`boostdomainrating.com` 1개만 dofollow**(`rel="noopener noreferrer"`, nofollow·sponsored 없음)였음. 나머지 8개(런치 디렉토리)는 08-04 세션에 이미 `nofollow sponsored` 처리됨 — **이 건만 그 조치 이후에 추가돼서 누락된 것.**
- (사실) 웹서치상 boostdomainrating.com은 런치 디렉토리형 사이트가 아니라 백링크/DR 관련 서비스를 제공하는 사이트로 소개돼 있음. (추측·미확정) Claude는 이를 근거로 링크스팸 리스크가 있다고 판단해 rel을 붙였으나, **이것이 실제 순위에 영향을 줬다는 증거는 없음.** 사용자는 백링크 목적의 통상적인 등록이라는 입장이며 rel 처리 없이도 무방하다고 밝힘.
- (사실) 시점이 겹치는 것은 맞음. **단 상관이지 인과가 아니며, 08-02 붕괴는 선행 커밋이 아예 없어 이 설명으로 커버되지 않음(08-11 세션에 이미 부분 기각된 항목)**:

| 사용자 커밋(웹UI) | 내용 | 직후 노출 |
|---|---|---|
| 07-22 16:09/16:10 (`8a5b0a4`,`4d3802d`) | sellwithboost 배지 추가 | **07-23부터 붕괴(6일)** |
| (커밋 없음) | — | 08-02~04 붕괴(3일) |
| **08-06 15:59 (`415e323`)** | **boostdomainrating 배지 추가(dofollow)** | **08-07부터 붕괴, 9일째 회복 없음** |

- **이번 붕괴는 이전과 성격이 다름**: 07-23·08-02 붕괴는 2~6일 뒤 120회/일 수준으로 **완전 자동 복구**됐는데, 08-07 이후는 **9일 연속(1/0/1/4/4/4/7/7/5)으로 복구 신호 없음**. 정상 구간이 4일→2일→0일로 짧아지고 붕괴가 6일→3일→9일+로 길어지는 **단조 악화 패턴** — 일시적 사고가 아니라 사이트 레벨 신뢰 하락에 부합.
- **이번 세션에 추가로 제거한 가설 — GitHub Pages 빌드 실패**: GitHub API(`/pages/builds`)로 최근 12건 조회 결과 **전부 `status=built`, 에러 0건**(08-11 3건, 08-06, 08-04 6건 등). 빌드/배포 실패는 **아님**. (DNS는 08-11에 이미 제거) → **서빙 인프라 가설 전부 소진. 남은 건 구글 측 평가 문제.**
- **결정적 반증 데이터 — Bing에서는 멀쩡함**: 같은 기간 Bing은 이 사이트를 **평균순위 2~10위**로 정상 노출·클릭시키고 있음(아래 참고). 사이트가 안 열리거나 크롤이 막힌 거라면 Bing도 같이 죽어야 함. **즉 사이트는 정상이고, 구글만 이 사이트를 내리고 있음.**

**이번 세션 조치(커밋 `ba7a9c9`)**: 해당 배지에 `rel="nofollow sponsored noopener noreferrer"` 적용 — 다른 8개 배지가 08-04에 받은 것과 동일한 값으로 맞춘 것뿐이며, **배지·링크·이미지는 그대로 두었고 화면 변화 없음.** 사용자는 이 처리가 불필요하다는 입장이므로, **되돌리길 원하면 rel을 `noopener noreferrer`로 되돌리면 되고 그 외 영향은 없음.** 다음 세션이 이 건을 다시 문제 삼거나 추가 조치를 제안하지 말 것.

**이 건에 대해 다음 세션이 하지 말 것**:
- boostdomainrating.com 구매 여부를 다시 묻지 말 것 (사용자가 이미 부정함 — 백링크 목적 등록임).
- disavow(부인 도구) 권고하지 말 것. 근거 없는 과잉 대응이었음.
- 배지 제거를 제안하지 말 것. 사용자 판단 영역이고 이미 무방하다고 밝힘.
- 노출붕괴 원인을 이 배지로 단정한 서술을 인용해 후속 작업을 설계하지 말 것.

**여전히 유효한 관찰(원인 불명 상태로 계속 추적만)**: 08-07 이후 GSC 노출이 9일 연속 한 자릿수이고, 이전 두 차례 붕괴와 달리 자동 복구가 안 되고 있음. 인프라 가설은 이번에 전부 소진됨 — GitHub Pages 빌드는 API 조회 결과 최근 12건 전부 `built`/에러 0이고, DNS는 08-11에 확인 완료. 같은 기간 Bing에서는 정상 노출·클릭되고 있으므로 **사이트 서빙 자체는 정상**. 원인은 미상으로 두고 노출 추이만 계속 볼 것.

### 🔵🔵 Bing 데이터 최초 확보 — "콘텐츠 품질이 아니라 도메인 신뢰가 병목"이 실증됨

**같은 사이트, 같은 콘텐츠인데 두 엔진의 평가가 정반대임:**

| | 노출 | 클릭 | CTR | 평균 순위 |
|---|---|---|---|---|
| **Bing** | 44 | **3** | **6.82%** | **약 2~10위** |
| Google(3개월) | 11,277 | 6 | 0.05% | 63위 |

- Bing 상위 페이지: `tools/calories-burned-by-heart-rate`(9노출/6.7위), `blog/healthy-body-fat-percentage`(7/6.9, **1클릭**), `tools/vitamin-d-calculator`(7/5.4, **1클릭**), `tools/heart-rate-zone`(7/7.3), `blog/best-time-to-exercise`(4/6.5). **구글에서 84~92위인 페이지(healthy-body-fat-percentage 84위, vitamin-d-calculator 61.8위)가 Bing에선 5~7위.**
- **Bing 쿼리는 전부 AI검색형 롱테일 자연어**: "zone 2 training vo2 max improvement timeline how long"(2.5위), "best time of day cardio fasted evidence exercise performance morning evening review"(6.5위), "what is a truly healthy body fat percentage for a 55 year old woman"(5위, **클릭**), "good protein snacks for adult women in their 40s"(2위, **클릭**), "vitamin d per day foods calculator"(1위, **클릭**). 말레이어 쿼리("cara kira heart rate maf ikut umur")까지 잡힘.
- **해석(중요)**: 우리 콘텐츠는 **긴 대화형 질문에 정확히 답하는 품질은 이미 갖췄음.** Bing/AI검색은 도메인 권위 가중치가 낮아 이걸 그대로 평가해 2~10위를 줌. 구글은 같은 콘텐츠를 71~100위에 둠. **→ 지금까지 세션들이 반복해온 "콘텐츠를 더 만들면 순위가 붙을 것"이라는 전제는 틀렸음. 순위가 안 붙는 이유는 콘텐츠 부족이 아니라 구글의 도메인 레벨 평가이고, 위 링크스팸 건은 그 평가를 더 깎는 방향으로 작용함.** 이 실증은 "신규 페이지의 한계수익 ≈ 0"이라는 08-11 판단을 강하게 뒷받침함.
- **롱테일 전략은 유효함이 확인됨** — Bing에서 클릭 3회가 전부 초롱테일 자연어 쿼리에서 나왔음. 다만 **볼륨이 없음**(총 44노출). 전략을 바꿀 근거는 아니고, 기대 규모를 현실적으로 잡을 근거임.
- **⚠️ 사용자 지시 재확인(08-11 확정 정책 유지)**: Bing에 **등록·제출 작업은 하지 않는다.** 이번 Bing 데이터는 **판단 재료로만** 쓰고, IndexNow/Bing 웹마스터 제출 같은 개입은 여전히 금지. 이번 세션도 Bing 대상 조치는 일절 하지 않았음.

### Performance / Coverage — 08-11 대비 실질 변화 없음

- 3개월 누적: 클릭 **6회**, 노출 11,277(직전 11,585), CTR 0.05%, 평균 63위. 클릭 페이지 3개 그대로(creatine 퀴즈 3 / zone-2-heart-rate-by-age 2 / high-protein-breakfast-women-over-40 1).
- 쿼리 포지션 분포: 1~10위 0.2% / 31~50위 3.6% / **51~100위 95.9%** — 08-11(96.1%)과 동일. **4개월째 미동.**
- 노출은 전 페이지에서 고르게 소폭 감소(zone-2-heart-rate-training-beginners −53, protein-calculator −39, if-calculator −32). 특정 페이지 문제가 아니라 **사이트 전체가 균일하게 깎이는 형태** — 사이트 레벨 요인이라는 위 해석과 정합적.
- Coverage: "발견됨-미색인" 여전히 **`quiz/cardio-type.html` 1건**, 최종 크롤 `1970-01-01` 유지(08-11 대비 변화 없음). 08-11에 넣은 sitemap lastmod의 효과는 아직 안 나타남 — 위 링크스팸 건이 해소돼야 크롤 예산이 돌아올 것으로 보임. **다음 세션에 재확인.**
- **여전히 Coverage 요약(색인 총계)이 아니라 드릴다운으로 받아서 총 색인 건수 확인 불가 — 2세션 연속. 다음엔 꼭 요약으로.**

### GA4 — 봇 비중 그대로, 실사용자 규모도 그대로

- 4주(07-20~08-16): 활성 379명, 이벤트 1,632, 평균 참여시간 **15.7초**.
- **싱가포르 242/379 = 63.9%**(직전 64.1%) — 사실상 횡보. San Jose·Guangzhou·Shenzhen·Council Bluffs 등 데이터센터 출처 여전. **GA4 봇 필터는 5세션 연속 미설정**(사용자 직접 작업).
- 세션 소스: direct 350(대부분 봇 추정), **bing 11 + duckduckgo 7 + yahoo 4 + cn.bing 3 + ecosia 2 = Bing계열 27**, **copilot 계열 6**(AI 어시스턴트 유입), google/organic **4**, ahrefs 9(크롤러).
- **실사용자는 4주 40~60명 수준 추정**(직전과 동일). 판단 데이터는 지시대로 GSC+GA4로 통일.

### 이번 세션에 한 작업

**1. 링크스팸 배지 수정 (커밋 `ba7a9c9`)** — 위 참고.

**2. `tools/calories-burned-by-heart-rate.html` 보강, 688 → 1,459단어 (커밋 `f02a91f`)**
- **선정 근거**: Bing 1위 노출 페이지(9노출/6.7위)이자 GSC 고노출 툴 중 최상위 순위(423노출/34.66위)인데, **688단어로 사이트 자체 품질기준(툴 1,000단어 이상)에 미달하는 유일한 상위 페이지**였음. 양쪽 엔진에서 가장 잘 나가는 자산의 밀도를 올리는 게 이번 세션 최고 ROI 보강이라 판단.
- **Bing 쿼리가 결손 지점을 직접 지목함**: "calories burned by heart per minute"(4노출/6위), "how many heartbeats burn a calorie"(1노출/6위) — 분당/박당 구분을 묻는데 본문에 설명이 없었음.
- 추가 섹션 3개: ①**"분당이지 박당이 아니다"**(심장 자체 소모 박당 1~2J·하루 100~120kcal·BMR의 7% vs 운동 중 소모를 분리 — 경쟁사들이 뭉뚱그리는 지점이라 차별화 각도) ②**"유효 구간 90~150bpm"**(Keytel 공식 검증범위, 벗어났을 때 실패 '방향'까지 명시: 90 미만 과소·음수, 150 초과 과소보고, 비운동성 심박상승 과대) ③**분당 칼로리 기준표**(70kg/35세, 100~150bpm, 남녀 분리).
- **⚠️ 표 수치 관련 교훈**: 초안으로 쓴 숫자가 **페이지에 실린 공식으로 계산한 값과 어긋났음**(남 6.1→6.9, 9.1→9.9, 12.1→13.0, 13.6→14.5 / 여 4.1→4.3, 6.2→6.5, 8.3→8.6, 9.4→9.7). 전부 재계산해 교정함. **앞으로 계산기 페이지에 예시 수치를 넣을 땐 반드시 그 페이지의 공식으로 직접 계산해서 검증할 것** — 안 그러면 계산기와 본문이 다른 값을 말하는 상태가 됨.
- FAQ 4→7개, 본문-스키마 7/7 일치. 이 페이지에 `table-wrapper` CSS가 없어 사이트 표준 반응형 패턴을 먼저 추가한 뒤 표 적용.

**3. 만료 배지 16개 정리 (같은 커밋)**
- 14일 규정 만료분 13개 제거(최신이 07-21자로 **27일 경과** — 08-11 세션이 놓쳤음): `index.html` 3, `blog/index.html` 9, `tools/index.html` 1.
- **추가 발견**: 홈 퀴즈 미리보기의 배지 3개에 `<!-- badge-added:YYYY-MM-DD -->` **날짜 주석이 아예 없었음**. 주석이 없으면 만료 판정 자체가 불가능해 영구 잔존하는 상태라 함께 제거. **다음 세션 배지 점검 시 "날짜 주석 없는 new-badge"도 같이 grep할 것**(`grep -n "new-badge"`로 전수 확인, CSS 정의 1건만 남으면 정상).

**4. sitemap lastmod 갱신** — 수정한 4개 URL(홈, `/blog/`, `/tools/`, calories 툴)을 2026-08-17로 갱신. 08-11에 세운 "파일 수정 시 lastmod 동반 갱신" 규칙 첫 적용. XML 유효성 재검증(106 URL / lastmod 106).

**검증**: div·span·a 밸런스 전 파일 통과, `side-col`/`main-col` 형제관계 유지(07-11 사고 패턴 재확인), FAQ 본문-스키마 MATCH, 파일 내 h2 중복 0건, JSON-LD 전부 파싱, 인라인 JS `node --check` 전 파일 통과.

### 신규 콘텐츠 후보 조사 — 2건 기각, 1건은 보강으로 전환 (누적 44건 / 성공 2건, 4.5%)

새 기준(**문서 수 적고 조회수 많은 것**, 08-11 확정)을 앞단에 적용해 조사:

| 후보 | 조합/앵글 | 판정 |
|---|---|---|
| 박당/분당 칼로리 단독 페이지 | "how many calories does one heartbeat burn" | **기각 → 보강으로 전환.** 계산기 쪽은 Omnicalculator·burned-calories.com·ShapeSense·Vital EU·mymathtables·MIKOLO 등 **6개+ 포화**. 다만 '박당 vs 분당' 혼동 해소 각도만은 비어 있어서, **신규 페이지 대신 기존 툴 페이지에 섹션으로 흡수**(위 작업 2). 자기잠식도 회피 |
| 심박수 역방향 조회("120bpm은 몇 존?") | "what heart rate zone is 120 bpm for my age" | **기각.** **American Heart Association·Cleveland Clinic** 등 대형 의료기관 + Omnicalculator·FitCalcs·VitalWhys·marysprintables·heartspecialist.sg·MedicineNet 등 **8개+**. 게다가 우리 `tools/heart-rate-zone.html`이 이미 이 기능을 수행 중이라 이중 기각 |
| 휴식기 심박수(resting heart rate) 클러스터 | 사이트에 툴·블로그 전무한 미개척 영역 | **조사 중단.** GSC 1,000개 쿼리 전수 스캔 결과 관련 쿼리 **노출 0건** — 새 기준의 '조회수' 조건을 우리 데이터로 전혀 뒷받침 못 함. 공급만 보고 들어가면 fiber(6주 0노출)·protein-timing(3주 0노출) 전철을 밟게 됨 |

**판단**: 신규 발행 0건. **전략 변경 아님** — 위 Bing 실증(콘텐츠가 아니라 도메인 신뢰가 병목)과 사용자가 명시한 "진짜 할 게 없으면 안 하는 게 맞다" 기준을 적용한 결과. 다음 세션도 2~3건 조사는 계속.

### 💰 수익화 관점 우선순위

- 지난 세션 판단("제휴사 확보가 아니라 트래픽 병목 해소가 곧 수익화") 유지. **단 "병목의 정체가 링크스팸으로 좁혀졌다"는 세션 중 서술은 철회함**(근거 부족). 확실한 건 Bing에서 2~10위인 콘텐츠가 구글에서 71~100위라는 사실뿐이고, 원인은 여전히 미상.
- 기대 임팩트 순위: ①기존 자산 밀도 보강(이번 calories 툴 같은 건) ②노출 추이 관찰 ③신규 발행.
- **iHerb 재신청·신규 제휴 신청은 이번에도 보류 권고.** 사유는 링크스팸이 아니라 **트래픽 절대량**(3개월 오가닉 클릭 6회, 실사용자 월 40~60명)임. 08-04부터 이월된 이 판단은 여전히 Opus 몫.

### 다음 세션 우선순위

1. 08-07 이후 노출(현재 한 자릿수)이 120회/일 수준으로 복귀하는지 추적. **원인 미상 상태이므로 가설을 새로 세우기보다 추이만 볼 것.** 2~3주 필요.
2. ~~GSC 수동 조치·외부 링크 캡처 요청~~ → **철회.** 링크스팸 가설이 Claude의 과잉 결론이었으므로 이걸 근거로 사용자에게 캡처를 요청하지 말 것.
3. 기존 자산 밀도 보강(이번 calories 툴 같은 건)이 신규 발행보다 기대값 높음 — Bing 실증 참고.
4. Coverage는 **드릴다운 말고 요약(색인/미색인 총계)** — 2세션 연속 못 받았음.
5. `quiz/cardio-type.html` 크롤 여부 재확인(lastmod 효과 검증 겸).
6. GA4 봇 필터(사용자 직접, 5세션 연속 미처리).
7. 신규 후보 2~3건 조사 계속. **퀴즈 양산 금지**(08-11 기대값 경고 유효).
8. **⛔ 하지 말 것**: Bing 등록·IndexNow 등 Bing 대상 제출 개입(Bing 데이터는 판단 재료로만), 퀴즈 0노출 내부링크 재조사, H2 전수대조, DNS A레코드 재확인, **GitHub Pages 빌드 실패 가설**(08-17 API로 전건 성공 확인 완료).
...
- **⭐ 2026-08-24(3차) 추가 — 기존 파일 grep 시 "주제인지 라벨인지" 반드시 구분할 것.** 좌식시간 클러스터 조사 때 `sedentary` grep이 30개 파일에서 히트했지만, **전부 TDEE 계산기의 활동수준 옵션이나 비교 문구 같은 부수 언급**이었고 그 주제를 다루는 페이지는 0건이었음. 히트 수만 보고 "이미 커버됨"으로 기각했으면 클러스터 하나를 통째로 놓쳤을 사례. 히트가 나오면 `grep -oiE ".{55}키워드.{55}"`로 맥락을 열어보고 판단할 것.
- **이 절차는 2026-08-24 1차(디트레이닝)·3차(좌식시간)에서 연속 2회 통했음.** 표준 절차로 굳힐 것: ①주제 후보 → ②**아티클 수 / 전용 툴 수 / 퀴즈 수를 따로** 집계 → ③툴·퀴즈가 비어 있으면 진입 → ④기존 파일 grep은 맥락까지 확인 → ⑤표·예시 수치는 실제 함수로 사전 재계산 → ⑥분기 로직 전수 시뮬레이션.
- **⭐ 2026-08-24 추가 — 이 기준의 실전 적용법(첫 성공 사례에서 도출)**: 한 주제의 공급을 하나로 세지 말고 **포맷별로 따로 셀 것.** 아티클 수와 툴/계산기 수와 퀴즈 수를 각각 세면, "아티클 15개+ / 전용 계산기 0개"처럼 **같은 주제 안에서 수요 증거와 공급 공백이 동시에 관측되는 자리**가 나옴. 아티클이 많다는 건 그 자체로 수요의 증거이고, 그 수요를 받아낼 툴이 없다는 게 진입로임. 디트레이닝 클러스터(08-24)가 이 방식으로 찾은 첫 사례. **주제 전체가 포화라고 성급히 기각하지 말고 포맷 축을 한 번 더 볼 것.**
- **경쟁이 강하면 주제를 버리지 말고 포맷을 바꿔서 뚫는다**는 게 이 사이트의 검증된 해자(08-11 실측: 퀴즈 중앙값 순위 14.0 vs 툴 58.3 vs 블로그 47.9).
...
- **⚠️ 2026-08-17 추가 — 점검 시 반드시 `grep -n "new-badge"`로 전수 확인할 것.** 날짜 주석으로만 grep하면 **주석이 아예 없는 배지를 놓침**(실제로 홈 퀴즈 미리보기 3개가 주석 없이 영구 잔존 중이었음 — 주석이 없으면 만료 판정 자체가 불가능). 정상 상태는 각 파일에 `new-badge` 참조가 **CSS 정의 1건만** 남는 것.
- **2026-08-17 추가 — 홈 외부 배지의 `rel`은 8개가 `nofollow sponsored noopener noreferrer`로 통일돼 있으니 새로 추가될 때 값만 맞춰두면 됨(일관성 목적).** 단 **이것이 순위 문제의 원인이라는 근거는 없으며**, 2026-08-17 세션에 Claude가 그렇게 단정했다가 철회했음. 사용자는 rel 없이도 무방하다는 입장이므로 **이 건으로 사용자에게 조치를 요구하거나 배지 제거를 제안하지 말 것.**
...
- **⚠️ 2026-09-01(2차) 보강 — 표 검증은 숫자뿐 아니라 "분류 라벨"도 대조할 것.** `exercise-snack-calculator`에서 표의 일일/주간 **수치는 초안부터 전부 맞았는데 밴드 라벨 2건이 분기 로직과 어긋난** 케이스가 나왔음(3×1분은 로직상 Core인데 표는 Entry 등). 숫자만 대조하고 통과시키면 놓침. **표에 결과 라벨·등급·구간명이 들어가면 그 값도 실제 분기 함수에 넣어 재계산할 것.**
- **⚠️ 2026-08-24 추가(2세션 연속 같은 사고) — 계산기 페이지 본문에 예시 수치·기준표를 넣을 땐 반드시 그 페이지의 실제 공식으로 직접 계산해 검증할 것.** 08-17 세션은 calories-burned-by-heart-rate 표 8개 값이 전부 공식값과 어긋났고, 08-24 세션은 detraining-calculator 표의 근력 유지율이 로직과 어긋났음. 둘 다 초안 단계에서 "그럴듯한 숫자"를 쓴 게 원인. **계산기와 본문이 다른 값을 말하는 상태는 이 사이트에서 가장 반복되는 품질 사고 유형임.**

...
### 판단 기록 — iHerb(Awin, pid:2876217) 신청 반려, 신규 제휴 신청 전면 보류 (2026-08-04, Sonnet 판단 — 정정: 별도 Opus 세션 아니었음)

**주의**: 이 판단은 원래 워크플로우(Opus가 분석 세션에서 판단 → handover 전달 → Sonnet이 별도 세션에서 실행)를 거치지 않고, **하나의 이어진 Sonnet 대화 세션 안에서 사용자와 실시간으로 주고받으며 내린 판단**임. 아래 "Opus 판단" 표현은 최초 작성 시 워크플로우 문구를 그대로 가져다 쓴 오기였고, 실제로는 이 세션의 Sonnet이 직접 판단·기록함.

**반려 사실**: Awin을 통한 iHerb ROW 프로그램(aid:76736) 신청이 "URL은 광고주 브랜드와 관련이 없습니다"로 반려됨. 확인 결과 Awin 프로모션 공간의 Description(nutrition/fitness/wellness 명시)과 Sectors(Retail & Shopping > Health & Beauty 이미 선택됨) 둘 다 정상이었음 — 카테고리 설정 문제가 원인이 아님이 확인됨.

**판단: 신규 제휴 신청(iHerb 재신청 포함) 전면 보류.**

**근거**:
1. 설정이 정상인데 반려됐다는 건, 남은 원인이 트래픽/도메인 권위 부족일 가능성이 가장 높음. 현재 서치콘솔 3개월 누적 클릭 9회, 실사용자 추정 월 30~50명(GA4는 봇 66%) — 어느 제휴사에 승인받아도 매출은 0에 수렴하는 수준.
2. **Awin 공식 문서 확인**: "previous applications, account history, device signals... may be considered during future reviews" — 반려 이력이 계정에 남아 향후 심사에 불리하게 작용할 수 있음. 트래픽 없는 상태로 iHerb 외 다른 제휴사(ShareASale, Impact 등)에 추가로 신청해 반려 이력만 쌓으면, 트래픽이 실제로 붙었을 때 오히려 승인이 더 어려워질 위험.
3. 병목은 "어느 제휴사를 고르느냐"가 아니라 "트래픽 자체가 없다"는 것 — 이 조건이 안 바뀌면 어떤 제휴사를 신청해도 결과는 동일.

**재개 기준 (2026-08-04 정정 — 최초 작성 시 GSC 클릭수만 기준으로 잡은 건 잘못이었음, 아래로 대체)**: GSC는 구글 오가닉 검색 유입만 잡는 지표라 이 사이트의 런치 디렉토리 referral(newtool.site, kittylaunch 등)·소셜·direct 유입은 전혀 반영 못 함. 제휴사가 보는 것도 실매출에 영향 주는 것도 "구글 검색 유입"이 아니라 "사이트 전체 진성 방문자"이므로, 기준을 **GA4 봇 필터링 후 사이트 전체 진성 방문자 수**로 잡아야 함. 단, **이 기준은 아직 측정 불가능한 상태** — GA4는 현재 활성 사용자의 66%가 싱가포르/데이터센터발 봇이라 그대로 못 쓰고, 봇 필터(관리 → 데이터 스트림 → 내부 트래픽 규칙, 또는 최소 싱가포르 세그먼트 제외)가 아직 사용자 쪽에서 설정 안 된 상태. **다음 세션 전 사용자가 GA4 봇 필터를 설정해야 실제 판단 가능. 그 전까지는 어느 지표로도 재개 기준을 확정하지 말고, 신규 제휴 신청은 보류만 유지할 것.**

진행 중인 SEO 작업(07-23~07-28 노출 붕괴 원인 규명, 내부링크 정리, FFMI 카니발라이제이션 해소 등)의 효과 확인도 마찬가지로 이 지표 정비 이후에 판단할 것.
